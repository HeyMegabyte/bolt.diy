...
const BUILD_SHA =
  (typeof process !== 'undefined' && (process.env?.GITHUB_SHA || process.env?.CF_PAGES_COMMIT_SHA)) || 'dev';

function hasValidKey(value: unknown): boolean {
  return typeof value === 'string' && value.trim().length > 10 && !value.includes('your_') && !value.includes('_here');
}
