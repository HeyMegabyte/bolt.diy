...
## Next target
(a) **cross-module `libs/features/*/handlers.ts` boilerplate** — jscpd shows the flag-gate + auth + error-envelope preamble clones across ~10 modules (referral_loop/token_burn_meter/prompt_studio/preview_share_card/site_doctor/payments_rail/observability_gateway/credit_wallet_rollover/cmdk_ai_actions). A shared `withOrgFlag`/handler-factory would DRY it, but each module varies → per-module diffing needed; batch across ~3 agents on disjoint modules OR a careful sequence, NOT one blind pass (learned iter 7: single agent on N files turn-limits). (b) more jscpd clusters in `src/routes`/`src/services`. (c) deeper social_publishers `publish()`/`fetchAnalytics()` tails (higher-risk, platform-interleaved). (d) knip unused exports/types — per-item vet, most built-ahead. sidebar BLOCKED. social.component split = fresh-context wave. Re-check constant FLAG_KEY drift after any flag change.
