...
## Next target
(a) ⚠️ **Brian decision (unchanged from iter 11)**: should hey@'s orgs get unlimited budget/builds? (1-line swap in the now-centralized `isUnlimitedOrgOwner`). (b) `mcp_oauth_provider/handlers.ts` self-dup (184 tokens, OAuth-sensitive — verify tests). (c) batched 11-module `unauthorized`/`notFound` (MESSAGE-DIVERGENT — fresh session, ~3 agents). (d) more jscpd clusters in routes/services. sidebar BLOCKED. social.component split = fresh-context wave. NOTE: 4 detector gates now guard convergence classes (idor, unwired, dbinsert, admin-email) — future finds ≥3-instance → add a `check-*.mjs` + fixture test + CI step.
