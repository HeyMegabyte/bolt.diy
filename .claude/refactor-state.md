...
| authed media / bare-nav (FE) | AUDITING (2 of 3 fixed) | The [[plain-navigation-cant-carry-bearer]] class beyond voice. iter 29: snapshot screenshots loaded via a plain `<img src="/api/…/screenshot.png">` (authed route → 401 → broken thumbnail) + a plain-nav PNG download. Fixed with a REUSABLE `AuthImageSrcDirective` (`img[appAuthImageSrc]` → `getBlobAbsolute` → object URL, revoked on change/destroy) applied to the thumbnail + modal, + download via `getBlobAbsolute`. 5 directive karma tests. FE-only (`main-3FNGE2UK`). ⚠️ REMAINING: docs-overview links the AUTHED `/api/admin/docs/openapi.json` in a `target=_blank` tab (verified 401) — needs popup-safe `window.open('')` + navigate-on-resolve. SAFE (verified, don't touch): OAuth `/api/auth/{google,github}` links (unauthenticated by design), public `/api/openapi.json` (200). |
...
## Next target
**Authed-media/bare-nav axis OPENED (iter 29)** — reusable `AuthImageSrcDirective` + `getBlobAbsolute` shipped; snapshot screenshots (thumbnail + modal + download) fixed (`main-3FNGE2UK`). Highest-value next:
- **Finish the authed-media axis** — 1 confirmed instance left: docs-overview's `target=_blank` link to the AUTHED `/api/admin/docs/openapi.json` (verified 401). Fix = a click handler that opens a blank tab synchronously (popup-safe) then navigates it to a `getBlobAbsolute` blob URL (async `window.open` after the fetch would be popup-blocked). Then re-sweep `social.component` `<img [src]="m.thumb_url||m.url">` — verify whether those URLs are authed `/api/media/…/raw` (→ use the directive) or public stock URLs (safe); apply the directive only to the authed ones.
