...
## Next target
(a) **systemic cross-module dup**: **11 modules re-define local `unauthorized`/`notFound` (+ a userId-only `guard`)** instead of importing canonical `feature_guard` (only site_analytics imports it): abuse_takedown, ai_gateway_guardrails, cmdk_ai_actions, credit_wallet_rollover, observability_gateway, payments_rail, prompt_studio, referral_loop, token_burn_meter, visitor_events_core, wireframe_planning. Consolidate — BUT first verify each module's message strings match feature_guard's (behavior), and batch across ~3 agents on disjoint modules (iter-7: single agent on N files turn-limits). (b) bounded single-file self-dups: `onboarding_copilot/handlers.ts`, `mcp_oauth_provider/handlers.ts` (jscpd self-clones, like iter 8 — safe). (c) more jscpd clusters. sidebar BLOCKED. social.component split = fresh-context wave.
