...
| voice conversations (FE↔worker) | CONVERGED (txt/vtt) | iter 26 added the MISSING `GET /api/voice/conversations/:id` (detail transcript). iter 27 added `GET /api/voice/conversations/:id/download.{txt,vtt,mp3,mp4}` (txt/vtt generated from `transcript_json`; mp3/mp4 302→`/recordings/:id/stream`) + rewrote FE `download()` to an authed blob fetch (new `ApiService.getBlob`) — a plain `<a download>` can't send the Bearer → every download 401'd. txt/vtt end-to-end; 11 jest + 17 karma green. ⚠️ mp3/mp4 buttons + `<audio>/<video>` still dark — detail route omits `recording_url` (buttons `[disabled]="!d.recording_url"`); next voice slice. |
...
- **Incomplete surfaces (FE built, backend missing — focused-session builds, NOT cron slices):**
  ...
  2. **voice conversation downloads** — ✅ DONE (iter 27) for txt/vtt: worker route `GET /api/voice/conversations/:id/download.{txt,vtt,mp3,mp4}` built + deployed (`a835e085`, unauth→401 live); FE `download()` now authed-blob fetch. ⚠️ REMAINING (next voice slice): mp3/mp4 download buttons + the `<audio>/<video>` player are still dark because the iter-26 detail route (`GET /api/voice/conversations/:id`) returns NO `recording_url`/`has_recording` → the buttons are `[disabled]="!d.recording_url"` + the players never render. The download ROUTE already serves mp3/mp4 (302→R2 stream); the gap is purely the detail route not surfacing recording presence + FE playback needing an authed-blob source (plain `<audio src>` can't send the Bearer either). Fix = detail route JOINs `voice_recordings` (has_audio/has_video) + FE reads a blob for playback.
...
## Next target
**Voice downloads BUILT (iter 27)** — worker route (txt/vtt/mp3/mp4) + FE authed-blob `download()` shipped + verified (`a835e085`). Highest-value next:
- **Finish the voice detail surface** (sibling of iters 26-27): make the iter-26 `GET /api/voice/conversations/:id` route JOIN `voice_recordings` and return `has_audio`/`has_video` (+ a recording id) so the FE can (a) enable the mp3/mp4 download buttons (`[disabled]="!d.recording_url"` today) and (b) render the `<audio>/<video>` player. ⚠️ Playback also needs an authed source — a plain `<audio src="/api/…/stream">` can't send the Bearer (same class iter 27 fixed for downloads); FE must fetch the recording as a blob (`ApiService.getBlob`) and bind an object URL. One coherent worker+FE slice + tests. This CLOSES the voice feature.
- **site-mcp tokens CRUD** — only when `site_mcp_server` is promoted (per CLAUDE.md convert-on-promotion); table + FE + contract ready.
