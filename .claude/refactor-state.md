...
## Next target
**Authed-media/bare-nav axis CLOSED (iters 29-30)** — all live instances fixed (snapshot screenshots + docs openapi), safe look-alikes audited (social=blob previews, OAuth=unauthenticated, public openapi). Reusable `AuthImageSrcDirective` + `getBlobAbsolute` now cover authed `<img>` + downloads/new-tab. Highest-value next (fresh axis — pick one):
...
- **New surface** — `orgs`/`hostnames` soft-delete edge queries, a fresh knip/jscpd delta, or another FE↔worker response-shape audit (a FE reader keying on the wrong response field silently shows empty, per [[response-key-mismatch-lying-empty]]). An honest verification checkpoint is legitimate if nothing safe+high-value surfaces.
