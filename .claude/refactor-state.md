...
## Next target
(a) ⚠️ **Brian decision (NOT auto)**: should orgs owned by `hey@megabyte.space` also get unlimited budget/builds? If yes → `isUnlimitedOrgOwner` swaps `=== 'brian@'` for `SYS_ADMIN_EMAILS.includes(...)` (one line, now centralized). (b) formalize the hardcoded-sysadmin-email DETECTOR (`[[audit-arc-detector-finds-bugs]]` — ≥3 instances now found: token_burn fixed + build_budget/build_limits DRY'd) → a `bin/` grep gate flagging `brian@`/`hey@megabyte.space` in an `=== `/`includes(` CHECK outside sysadmin.ts. (c) `mcp_oauth_provider/handlers.ts` self-dup (184 tokens, OAuth-sensitive — verify tests). (d) batched 11-module `unauthorized`/`notFound` (MESSAGE-DIVERGENT). sidebar BLOCKED. social.component split = fresh-context wave.
