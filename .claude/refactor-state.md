...
## Next target
(a) knip **"unused exports" (16) + "unused types" (25)** — vet a FEW HIGH-confidence ones/iteration (types are runtime-erased → low-risk), but MOST are built-ahead public-API surface → KEEP, never mass-remove (`[[knip-unused-not-always-dead]]`). Spot-check candidates: `search.ts: pickSafeRedirect`, `registry.ts: getDefaultFlag` — grep for callers before touching. (b) `@types/jszip` unused devDep — confirm jszip itself is unused before removing. (c) another cold worker-side improvement verifiable by Jest. sidebar BLOCKED (concurrent session owns `admin.component.*`/`nav-icon`/`admin-navigation-responsive.e2e.ts`). social.component split = fresh-context wave (Browserbase 6bp visual gate; read the 3267-line component + plan extract-to-subcomponents). Re-check constant FLAG_KEY drift after any flag change (registry.ts/docs.ts).
