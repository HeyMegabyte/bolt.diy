...
## Next target
(a) `mcp_oauth_provider/handlers.ts` self-dup (jscpd [228-237] vs [71-80], **184 tokens** — BIGGER clone than onboarding → likely a net reduction; OAuth-sensitive, verify its tests first). (b) the systemic **11-module `unauthorized`/`notFound`** consolidation — MESSAGE-DIVERGENT: a fresh batched session decides the canonical message + updates any module test asserting error.MESSAGE (most assert error.CODE only → likely safe); batch ~3 agents on disjoint modules. (c) more jscpd clusters in routes/services. sidebar BLOCKED. social.component split = fresh-context wave. Re-check constant FLAG_KEY drift after any flag change (none this iter).
