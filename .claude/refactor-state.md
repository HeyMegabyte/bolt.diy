...
## Next target
(a) ⚠️ **Brian**: hey@ unlimited? (1-line, centralized `isUnlimitedOrgOwner`). (b) **batched 11-module `unauthorized`/`notFound` → `feature_guard`** — a REAL behavior change (adds `request_id` to error bodies + unifies referral_loop's message) — an IMPROVEMENT but needs a FRESH session with ~3 agents + per-module test verification (tests asserting the exact error object break; most assert error.CODE only, so mostly safe). (c) more jscpd clusters in `src/routes` + `src/services` (the feature-handler clones are largely picked over now). sidebar BLOCKED. social.component split = fresh-context wave. NOTE: 5 detector gates guard convergence classes (fitness, idor, unwired, dbinsert, admin-email) — future ≥3-instance finds → add a `check-*.mjs` + fixture test + CI step.
