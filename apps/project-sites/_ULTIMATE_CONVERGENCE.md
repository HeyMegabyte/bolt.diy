...
- **🟡 VISIBLE-UX DEBT — first payment made (#17 preview section, 05938f46).** `/admin/bulk-ops` shipped: operation picker + set_flag fields + Preview → `POST /api/sites/bulk` dryRun → rolling-counter eligible count + skip table. Read-only (apply = follow-up w/ confirm dialog). 6/6 Karma, tsc 0. REMAINING visible surfaces (now cheap per the mapped mechanism): #12 deliverability score panel, #4 approval review page, an apply-with-confirm action on bulk-ops.
