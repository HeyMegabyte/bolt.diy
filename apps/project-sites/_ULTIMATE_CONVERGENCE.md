...
- **#4 review_approval_links** — 🟢 BACKEND-COMPLETE: state-machine core (a643561b) + `recordReviewDecision` (8b001cc5) + **public reviewer routes** (56bf60e5: `GET /api/review/:id` + `POST /api/review/:id/decision`, bearer=UUID id, flag-scoped to `agency_org_id`, 17/17). NO migration (`review_tokens.decision`/`used_at` reused). Remaining: a visible `/review/:id` reviewer page (stakeholder UI) + admin create-link surface; hardening (token_hash verify + rate-limit the public POST).
