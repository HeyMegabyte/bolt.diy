...
- **🖥️ VISIBLE-UX DEBT + admin-shell mapping prerequisite (standing).** The loop has shipped many dark worker-backed slices (#12/#17/#4/#10 are backend-only, flag-dark — zero visible change). The doc §2 "≥1 visible cyan/black UX per round" is unmet. Blocker: the admin shell (`frontend/.../admin/admin.component.ts`, 608 lines, swarm-hot) renders sections via a non-obvious mechanism (`currentSection` signal + directly-imported section components, no locatable switch/nav array) — hacking a new section in blind risks breaking the LIVE admin. **Before any admin-surface slice: map the shell first** (how `currentSection` → which component renders; where the nav list lives; route guards) in a dedicated session, THEN add the bulk-ops/deliverability/approval preview sections (cyan/black, Karma-tested, R2-deployable — frontend has no Docker gate). Do NOT force a section into the shell mid-5-min-round.
