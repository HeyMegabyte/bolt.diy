...
- **#10 outbound_webhooks** — 🟡 delivery-policy core (8187b991) + `webhook_endpoints` table (migration **0534**, encrypted `secret_encrypted`) + subscription validation gate (3611b7d5: `validateEndpointInput` https-only + `WEBHOOK_EVENT_TYPES` allowlist + `maskSecret`, 14/14). Slice 2b = `createWebhookEndpoint`/list/delete (encrypt via `ai_crypto` → store the single blob; `secret` shown once) + routes (auth+flag+`assertSiteOwned`) + flag + manifest. Slice 3 = dispatch (fan-out on events, sign + retry via the delivery-policy core; add `webhook_deliveries` + SSRF host-block).
