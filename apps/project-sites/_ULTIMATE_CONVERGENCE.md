...
- **#10 outbound_webhooks** — 🟢 CRUD-COMPLETE: delivery-policy core (8187b991) + `webhook_endpoints` table (migration 0534) + validation gate (3611b7d5) + encrypt-backed CRUD service (f1011c27: `createWebhookEndpoint`/list/delete, AES-GCM secret, shown once) + routes `GET/POST/DELETE /api/sites/:siteId/webhooks` (017a72c6, flag `outbound_webhooks`, 18/18). Slice 3 = dispatch fan-out (sign+retry via the delivery-policy core, `webhook_deliveries` table). **SSRF host-block DONE (5222e880): `isSafeWebhookUrl` — https-only + blocks localhost/.local, private/reserved IPv4 (incl. metadata 169.254.169.254), IPv6 loopback/link-local/ULA; the dispatcher MUST call it before every fetch (DNS-rebinding/connect-time IP-pin still TODO).** **All 6 Brian-prioritized modules are now full-stack or CRUD-complete (beyond core).**
