...
Closed: `content.ts` approve · `conversational_edits.ts` (4f7620be) · `swarm.ts` (e6cc063d, +auth) · `pseo.ts` (68cd750d — LIVE, 6 handlers) · `reviews.ts` (3daf721b — dormant, 3 handlers).
**Verified ALREADY-GUARDED (do NOT re-audit):** `mcp_site.ts` (`assertSiteOwnership` on all 7), `site_branches.ts` (`assertOwner` on all 6). The `own=0` scan signal is UNRELIABLE — helpers use varied names (`assertOwner`/`assertSiteOwnership`/`row.org_id !== orgId`); the reliable gap-finder is "`:siteId` route file with ZERO ownership-check signature anywhere" → currently: `ai_components.ts` (2h), `copilot.ts` (3h) — both flag-gated dormant, next targets. `features.ts` (31h) is EXCLUDED — per `apps/project-sites/CLAUDE.md` gotcha #10, do NOT mass-retrofit; guard per-feature only as each flag is promoted.
**Lessons (fold forward):** (a) **prioritize routes with NO feature-flag gate** — those are LIVE (any authed user), strictly worse than flag-off dormant ones; grep `routes/*.ts` for `:siteId` handlers lacking BOTH `isFlagOn` AND `assertSiteOwned`. (b) **mutations must scope by the parent `site_id`, not the child id alone** — `UPDATE … WHERE id = ?` on a child row (pseo_pages, changeset_files, etc.) lets a verified-owned `:siteId` still mutate a foreign row by guessed child id; always `WHERE id = ? AND site_id = ?`.
