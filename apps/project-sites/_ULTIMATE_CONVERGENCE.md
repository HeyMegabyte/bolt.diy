...
- **P1 build-first modules (#4/8/10/11/12/17) need a DEDICATED focused pass, not a single 5-min loop slice.** Friction found: `npm run gen:feature` requires a **kebab-case** slug while the codebase uses **snake_case** flags/dirs (resolve before scaffolding); modules are **worker-backed** (unverifiable locally — Docker-gated; ship on push); the pre-commit drift hook is **all-or-nothing** (manifest+flag+tests must all cohere); data-backed modules need a **D1 migration** (number races the swarm — claim atomically). **Lowest-risk first pick = `email_deliverability_wizard`** — stateless SPF/DKIM/DMARC checks via DNS-over-HTTPS, **no new table/migration**, naturally uses `assertSiteOwned`. Do it in a dedicated pass (or swarm task), not a quick loop round.
- **P2 state-primitive contract coverage (in progress):** `empty-state` (9596424e) ✓ · `error-card` (b1dd9d92) ✓ · **`skeleton` remains**. All three verified already production-grade (token-driven, focus-visible, reduced-motion, ARIA) — coverage locks the contract; do NOT invent cosmetic "visible" churn on already-excellent primitives.
