...
- **#10 outbound_webhooks** — 🟢 CRUD-COMPLETE: delivery-policy core (8187b991) + `webhook_endpoints` table (migration 0534) + validation gate (3611b7d5) + encrypt-backed CRUD service (f1011c27: `createWebhookEndpoint`/list/delete, AES-GCM secret, shown once) + routes `GET/POST/DELETE /api/sites/:siteId/webhooks` (017a72c6, flag `outbound_webhooks`, 18/18). Slice 3 = dispatch fan-out (sign+retry via the delivery-policy core, `webhook_deliveries` table, **SSRF host-block** before fetch). **All 6 Brian-prioritized modules are now full-stack or CRUD-complete (beyond core).**
