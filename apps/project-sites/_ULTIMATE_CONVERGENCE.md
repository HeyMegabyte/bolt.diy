...
- **#4 review_approval_links** — 🟡 state-machine core (a643561b) + **DB-backed `recordReviewDecision`** (8b001cc5, 15/15). ⚠️ NO migration — `review_tokens` ALREADY has `decision` (nullable) + `used_at` columns (0501); state derives from `decision IS NULL`→pending. Slice 2b = public token-bearer routes (`GET /review/:id` + `POST /review/:id/decision`) — security-nuanced (verify token vs `token_hash`, flag-gate `approval_workflow`).
