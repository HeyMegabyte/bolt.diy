...
Closed: `content.ts` approve (already fixed, verified) · `conversational_edits.ts` (4f7620be) · `swarm.ts` (e6cc063d — +missing auth) · `pseo.ts` (68cd750d — **LIVE, no flag gate, 6 handlers; +scoped approve/reject UPDATE to `site_id`**). **Still-suspect (verify each, fix with `assertSiteOwned`):** `reviews.ts` (9h/3, flag-gated dormant), `comparison_pages.ts` (5h/2), `integration_directory.ts` (5h/2), `site_branches.ts` (12h/4), `site_dna.ts` (9h/5), `copilot.ts` (9h/6). Read each first — some may already guard differently; verify per CONVERGENCE.md §6.
