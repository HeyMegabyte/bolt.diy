...
- **P1 #12 `email_deliverability_wizard` — CORE LANDED (407de810).** Strategy that worked in a loop round: build the module **bottom-up, vertical-slice by slice**, NOT all-at-once via `gen:feature`. Round 1 shipped the **pure, injectable-`fetch` core** (`src/services/email_deliverability.ts` + 6 green unit tests) — additive new files, **zero drift-hook/migration/Docker exposure**, fully verifiable locally. The score model: SPF 35 + DMARC 35 + DMARC-policy(quarantine|reject) 10 + DKIM 20 = 100; DNS-over-HTTPS to `cloudflare-dns.com`; common DKIM selectors probed; graceful `score 0` on lookup failure. **NEXT slice (this is the worker-backed part — commit, ships on push):** `GET /api/sites/:siteId/deliverability` route → `auth` + `isFlagOn('email_deliverability_wizard')` + **`assertSiteOwned`** + resolve the site's primary custom hostname's domain → `checkDeliverability(fetch, domain)`; then the flag row in `src/modules/feature_flags/registry.ts` (`enabled=0, rollout=0, stage='experimental'`) + `libs/features/email_deliverability_wizard/feature.manifest.ts` (mirror the `review_synthesis` registry-entry-only pattern: code in `src/`, manifest declares `unitTests:['src/__tests__/email_deliverability.test.ts']`, `e2eTests:[]`) + mount in `src/index.ts`. Run `npm run validate:features:quick` before that commit (drift hook is all-or-nothing).
- **Lesson (fold forward): build worker-backed P1 modules bottom-up in the loop.** Slice 1 = pure core + unit tests (additive, safe, verifiable now). Slice 2 = route+guard+flag+manifest+mount (drift-coherent, ships on push). This beats both "force `gen:feature`" (kebab/snake friction) and "defer entirely to a dedicated pass" — the core is real, committed, tested progress every round.
- **Older note (still true):** `gen:feature` wants a **kebab-case** slug vs the codebase's **snake_case**; data-backed modules need a **D1 migration** (number races the swarm — claim atomically); worker code is unverifiable locally (Docker-gated, ships on push).
