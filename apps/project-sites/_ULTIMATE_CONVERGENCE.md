...
- **🟡 P1 #17 `bulk_site_ops` — CORE LANDED (5f53f275), slice 1 of 2.** Pure `planBulkOperation` (`src/services/bulk_site_ops.ts`) + 7 green unit tests. It's the safety heart of "apply to ALL sites": ownership filter (`not_owned` skip → never cross-tenant), per-op validity (`republish`=published-only, `archive`=non-archived, `set_flag`=any non-archived), `MAX_BULK_SITES=100` cap (overflow → `batch_cap_exceeded`, never silent), dedupe. Migration-free, additive. De-dup verified clean (the 3 "bulk" hits in env_vars/api/super_admin are dotenv-import / file-export / cache-purge — NOT apply-across-sites). **NEXT slice (worker-backed, ships on push):** `POST /api/sites/bulk` → auth + `isFlagOn('bulk_site_ops')` + resolve caller's org-scoped sites from D1 (`WHERE org_id = ?`) → `planBulkOperation` → execute ONLY `eligible` per-op (set_flag → `feature_flag_overrides`; republish → existing publish path; archive → soft-archive) → return `{ plan, results }`. Flag in `registry.ts` (experimental, off) + registry-entry-only manifest (`unitTests:['__tests__/bulk_site_ops.test.ts']`) + mount. Keep slice-2 synchronous + capped at 100 → **no migration needed**. `validate:features:quick` before staging.
