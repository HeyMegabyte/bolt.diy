...
- **⚠️ review_approval_links (#4)** → EXTEND the existing **`approval_workflow`** flag + **`review_tokens`** table (the `createReviewLink` demo stub at `features.ts:402` / `services/features.ts:541`). Do NOT fork a new `review_approval_links` flag/table — that would duplicate. `review_tokens` today = `(id, site_id, agency_org_id, token_hash, expires_at)` with **NO status column** → slice 2 = `ALTER TABLE review_tokens ADD COLUMN status` (default 'pending') + approve/reject/comment routes adopting `services/review_approval.ts` guards + promote the stub off its `'demo-site'`/`'demo-agency'` defaults + auth/ownership. The pure state machine (a643561b) is flag-agnostic so it's safe to land before the flag decision.
