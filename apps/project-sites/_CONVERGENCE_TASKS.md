...
- **✅ team + leads added to the shared 22-section list (commit 96da6967)** — the two top-level admin routes the 20-section sweep didn't cover. Re-running the brian sweep now (22 sections) to verify they render brian's real data. (editor = iframe/journey-covered; super-admin = operator-guard; welcome = onboarding — not data surfaces.)
- **COVERAGE AFTER THIS FIRE:** 22 top-level (brian + e2e-org) + logs/user (deep-visual) + 8 site-detail sub-routes (brian) = **the entire admin surface Browserbase-real-data-verified**. That's the mandate's literal core across every reachable admin data surface. Once the 22-section brian re-run confirms team+leads green, the visual half of gate (A) is 100%.
