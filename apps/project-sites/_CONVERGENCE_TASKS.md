...
- [ ] **⏭ PASS-9 QUEUE (from Pass-8 fold):**
  - **9 pre-existing jest suites fail on main (NOT from this fold's diff; diverse areas broke together → likely ONE shared root cause):** `outbox_dispatch`, `feature_flags_docs`, `wrangler_class_bindings_lockstep`, `social_publish_workflow`, `services/file_cleanup`, `forms_routes`, `features_routes`, `marketing_serve_injection`, `capability_router`. Repro: `npx jest <file> --silent`. (`debug_rate.test.ts` — a committed debug script with `process.exit` — deleted this pass.)
  - **`X**` glob audit sweep:** ~20 more `route('**/…X**')` patterns in currently-green specs (`admin-feature-flags` super-admin `**`, `admin-system-services` services `**`, `media-podcast` generate `**`, create-wizard search `**`, …) — vet each against subpath traffic; convert to `X/**` (+ exact `X` where the bare path is called) where the endpoint has children. Detector: `grep -rnE "route\('\*\*[^']*[a-z]\*\*'" e2e`.
  - Skip-triage actions: DELETE 107 stale hard-skips + REVIVE 2 (`site-mcp/site-mcp.spec.ts:116`, `branches/branches.spec.ts:82`) per `e2e/_skip-triage-2026-07-31.md`.
  - (a) `#create-address` lacks `maxlength` — FE accepts 330+ chars, only server enforces (add maxlength=500 to match settings); (b) site-features/site-detail specs green but SOFT-ASSERTED (`if visible` guards) — add missing `sf-*`/`sd-tab-*` testids then stricten; (c) 2 create-suite `test.fixme` races (post-submit auth flow) — revisit with the now-working real-session pathway; (d) BA-cutover follow-ups: backfill remap for email-collision users (mirror skips + warns today) + consider the BA bridge for the legacy `signin/signin.component.ts` page too.
