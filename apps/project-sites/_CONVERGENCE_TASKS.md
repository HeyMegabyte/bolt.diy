...
- [ ] **Wave-4 TDD-RED specs (next fire's queue — 80 failing tests across 6 files, excluded from prod gate via testIgnore):** `admin-editor-journey`, `admin-snapshots-journey`, `admin-site-features-journey`, `admin-site-detail-journey` (~6 failures each — likely stub-shape/selector calibration like wave-2/3), `value-domains-create` + `value-domains-settings` (large suites, shared navigation-timeout cause; `networkidle`→`domcontentloaded` already applied — recalibrate selectors per value-domains-auth lessons: real testids, disabled-button-as-rejection contract, blur-triggers-validation). Green them, then remove the six `testIgnore` entries in `playwright.prod.config.ts` and flip COVERAGE.yml `blocked`→`covered`.
