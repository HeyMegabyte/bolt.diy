...
- **RANKED REMAINING toward DONE:** (a) a per-input value-domain pass (TDD #10: valid/invalid/empty/boundary/overlong/unicode/injection) on the key admin forms in real Chrome; (b) site-detail sub-routes (branches, mcp-server, features) + editor; (c) a nightly `RUN_BROWSERBASE=1` CI job (creds as repo secrets) for unattended visual+a11y regression across all 20.
