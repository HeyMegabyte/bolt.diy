...
- [ ] **⏭ PASS-7 FOLD CHECKPOINT (do FIRST next fire — 4 agent worktrees hold finished/near-finished work):**
  ...
  - ✅ `agent-a66351db202001fa4` FULLY DONE — google-flake fixed (root cause: `domcontentloaded` on a 302-redirecting URL races the redirect under parallel load → `net::ERR_ABORTED`; fix: prime SPA → `load`+try/catch → waitForURL off `/api/` → expect.poll body length; verified 3/3 ×2 workers) AND skip/fixme triage complete → **`e2e/_skip-triage-2026-07-31.md`** (in-repo). Action next fire: DELETE 107 stale hard-skips (adversarial-suite mass-skips dominate; also confidence-ui:20, rebuild-and-research:222, ai-endpoints-ide ×6), REVIVE 2 wrongly-formed `test.skip(title)` tests (`site-mcp/site-mcp.spec.ts:116`, `branches/branches.spec.ts:82` — features live, auth available), KEEP the 24 conditional runtime guards.
