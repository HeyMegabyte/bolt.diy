...
### P0.12 (fire 2026-08-02l) — ✅ site-detail SUB-routes swept for BRIAN (8/8 green) + team/leads added to top-level sweep → full admin-surface Browserbase coverage
...
- **⚠️ team+leads real-data verification is INFRA-BLOCKED right now (transient Browserbase API), NOT a content problem.** The 22-section brian re-run flaked twice: (1st) a mid-sweep session DROP at ~section 18 ("Target page has been closed") → fixed with **`retries:1` + 900s session** on both long sweeps (commit 76bb000b — the correct fix for non-deterministic session drops); (2nd, after the fix) BOTH attempts failed at session-CREATE with `ConnectTimeoutError: api.browserbase.com:443 timeout 10000ms` — i.e. after ~5 rapid Browserbase sessions this session, the API is transiently unreachable / rate-limiting. team+leads NEVER rendered in either run, so they're simply UNVERIFIED-YET, not broken. This is a genuine external blocker per [[error-recovery]] (external-down → backoff → note) — resolves on the nightly `prod-visual-browserbase` CI run (now retries-hardened) or the next fire when the API is reachable. The retries:1 hardening ALSO makes the nightly job self-heal the mid-sweep-drop class.
- **COVERAGE THIS FIRE (verified green):** **30 admin surfaces for BRIAN** — 20 top-level (P0.11) + logs/user (deep-visual) + 8 site-detail sub-routes (P0.12). PENDING (infra-blocked): team + leads (2) via Browserbase, + super-admin (operator-only, needs a brian-ONLY sweep slot since it redirects non-operators — can't share the e2e-org list). Visual half of gate (A) ≈ 30/33 surfaces. **(B) 660 prod-enrolled E2E ≥ 400 ✓.** Cron stays armed for team/leads/super-admin.
