 ...
 * Filters CF Access bypass paths and static assets locally.
 */
import { Hono } from 'hono';

const app = new Hono();

// Static assets + PWA manifest served locally (don't go through CF Access → CORS fails)
app.get('/manifest.json', (c) => c.json({
  name: 'ProjectSites Integrations',
...
app.get('/favicon.ico', async (c) => {
  try {
    const resp = await fetch('https://projectsites-nango.fly.dev/favicon.ico');
    if (resp.ok) return resp;
  } catch {}
  return new Response(null, { status: 204 });
});

// Everything else → proxy to Fly.io Nango with 45s timeout (cold starts)
app.all('*', async (c) => {
  try {
