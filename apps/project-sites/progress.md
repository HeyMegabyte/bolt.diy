...
- **Meta-lesson (fires 23, 28, 31):** I have repeatedly declared "frontier exhausted" and been wrong by dismissing a category without inspecting. DEFAULT ASSUMPTION should be "there is more clean coverage work" — only no-op after a concrete inspection turns up nothing. Next fire: publishers all done — INSPECT a workflow/loader/route target for tractable testable logic (prefer extractable pure helpers or a DI-able async fn like the f26 `resolveZoneForHostname`). If a target is pure-orchestration with no clean seam, note it + try the next; only no-op after a real inspection of the remaining heavy targets turns up nothing clean.
