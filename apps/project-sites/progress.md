...
## ⚑ CONVERGENCE STATUS — read FIRST (updated fire-33, 2026-06-11)
...
- **ACTIVE autonomous backlog (one per fire, replicate the twitter pattern — but VERIFY each publisher's own shape first; they differ):** DONE: twitter (f31), linkedin (f32), facebook + instagram (f33). REMAINING 7: **threads (next)**, bluesky, reddit, mastodon, discord, slack, telegram. Then the heavier: workflows (`site-generation` 9%, social-publish, snapshot-quality, pseo), DB/GraphQL loaders (`loadMultiUrlAnalytics`/`listSiteUrls`), HTTP routes (~50%).
- **Meta-lesson (fires 23, 28, 31):** I have repeatedly declared "frontier exhausted" and been wrong by dismissing a category without inspecting. DEFAULT ASSUMPTION should be "there is more clean coverage work" — only no-op after a concrete inspection turns up nothing. Next fire: cover `threads` (next publisher) — VERIFY its shape first (publishers differ widely: linkedin=UGC+external_id; facebook=page-feed; instagram=2-step container+requires-image).
