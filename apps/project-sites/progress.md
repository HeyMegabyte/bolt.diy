...
## ⚑ CONVERGENCE STATUS — read FIRST (updated fire-36, 2026-06-11)
...
- **ACTIVE autonomous backlog (one per fire, replicate the twitter pattern — but VERIFY each publisher's own shape first; they differ):** **ALL 11 PUBLISHERS DONE** (twitter f31, linkedin f32, fb+ig f33, threads+bluesky f34, reddit+mastodon f35, discord+slack+telegram f36) — `social_publishers/*` category complete + the shared `types.ts` (f28). **NEXT frontier = genuinely heavier:** workflows (`site-generation` 9%, social-publish, snapshot-quality, pseo — `WorkflowEntrypoint` classes, need a step-runner harness), DB/GraphQL loaders (`loadMultiUrlAnalytics`/`listSiteUrls` — mock DB + per-host GraphQL), HTTP routes (~50% — full app harness). VERIFY tractability per-target (some workflow `run()` methods may be too orchestration-heavy for clean unit value — extract-able pure helpers within them are the better target if present).
