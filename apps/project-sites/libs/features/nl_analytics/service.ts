...
const PATTERNS: Pattern[] = [
  ...
  { regex: /how many (builds|deploys)/i, sql: "SELECT COUNT(*) as value FROM workflow_jobs WHERE org_id = ? AND type='build' AND deleted_at IS NULL", explain: "Counting all builds in your org" },
  { regex: /(builds|deploys) this month/i, sql: "SELECT COUNT(*) as value FROM workflow_jobs WHERE org_id = ? AND type='build' AND created_at >= datetime('now','start of month') AND deleted_at IS NULL", explain: "Counting builds created this calendar month" },
  { regex: /most active site/i, sql: "SELECT s.name as label, COUNT(w.id) as value FROM sites s LEFT JOIN workflow_jobs w ON w.site_id = s.id AND w.deleted_at IS NULL WHERE s.org_id = ? AND s.deleted_at IS NULL GROUP BY s.id ORDER BY value DESC LIMIT 1", explain: "Finding the site with the most workflow jobs" },
