...
export async function answer(
  ...
  const chunks = await semanticSearch(env, message, { topK: 5, orgId: siteId }).catch(() => []);
  const context = chunks.map((c: { text?: string; content?: string }) => c.text ?? c.content ?? '').filter(Boolean).join('\n\n');
  ...
  try {
    const result = await (env.AI as { run: (model: string, params: { messages: { role: string; content: string }[] }) => Promise<{ response?: string }> }).run(MODEL, {
      messages: [
...
export async function getConfig(siteId: string): Promise<{ siteId: string; enabled: boolean; greeting: string }> {
