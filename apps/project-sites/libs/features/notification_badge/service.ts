...
export async function getBadgeCounts(env: Env, orgId: string): Promise<{ total: number; alerts: number; builds: number }> {
  const [alerts, failedBuilds] = await Promise.all([
    dbQueryOne<CountRow>(env.DB, `SELECT COUNT(*) as cnt FROM audit_logs WHERE org_id=? AND action LIKE '%.failed%' AND created_at > datetime('now','-7 days') AND deleted_at IS NULL`, [orgId]),
