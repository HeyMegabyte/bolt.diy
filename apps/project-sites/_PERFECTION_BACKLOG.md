...
- 🔴 **NEXT-WORST = homepage LCP 9.4s on 3G (P0).** An 18KB text page should LCP ~0.5-1s — the largest paint is gated by a late resource. Diagnose the LCP element (no hero `<img>` in source → suspect the Lottie animation or an image injected after the Uppy/Lottie CDN JS executes; the SPA's CDN deps (Uppy ~big, Lottie) are render/paint-gating). NEXT FIRE: capture the LCP element (Playwright `largest-contentful-paint` entry `.element`/`.url`), then defer/async the CDN JS + `fetchpriority=high`+preload the LCP visual (or replace Lottie hero with a CSS/inline-SVG paint), redeploy, re-measure → flip the gate to blocking when LCP≤2000.
