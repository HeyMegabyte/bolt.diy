...
- 🟢 **HOMEPAGE NOW ALL-GREEN with wide margin: LCP ~450-570ms · FCP ~443-492ms · CLS 0.002 (all 3 ≤ target).** Held 1 fire (fire 8). **GATE re-enable: ONE more all-green fire (fire 9) → re-enable `e2e/perf/ttfr.spec.ts` in `playwright.prod.config` testMatch as a BLOCKING gate** (fire 9: re-measure homepage to confirm hold, flip the testMatch line, push). LCP+CLS were green since fire 5; FCP joined fire 8. Run on demand meanwhile: `npx playwright test e2e/perf/ttfr.spec.ts --config playwright.prod.config.ts`.
