...
- ⚠ 2026-06-23 (fire 11) **FIRST Lighthouse/INP measurement — reveals the interactivity ceiling the paint-only gate hid.** Homepage Lighthouse (mobile, perf): **score 66 ❌** (target ≥90). Paint is great (FCP/LCP 1.2s ✅, CLS 0.004 ✅) BUT **TBT=3010ms ❌, TTI=11.3s ❌, Speed-Index 5.9s** — the CSR Angular SPA boots a large EAGER JS payload that blocks the main thread (so INP/responsiveness is poor even though paint is fast). Eager chunks: **`chunk-GGAROBNS.js` = 784KB (ag-grid — admin log grids ONLY, NEVER used on marketing)** + `chunk-K2V7VFOW` 584KB (Sentry+vendor) + 240/192/180KB others. **The ttfr paint gate (LCP/FCP/CLS) is green + locked, but it does NOT cover TBT/INP — that's the real remaining homepage gap.** Two levers (both substantial, neither a safe 1-fire ship — surfaced, not rushed):
  - **(1) perf-wave ag-grid→TanStack** (`docs/perf-wave-ag-grid-to-tanstack.md`): migrate `audit.component.ts` + `ai-logs.component.ts` (the ONLY ag-grid users) to the existing `createAngularTable` pattern (already proven in api-tokens + content-freshness). Removes 784KB eager from EVERY page incl. marketing → est. TBT ~3010→~1700ms, score ~66→~75-80. BOTH components must migrate (esbuild hoists the shared dep to eager even from one lazy route — `@defer`/lazy tricks are documented DEAD ENDS). Multi-component + master/detail (full-width rows) + CSV + dark theme + pagination reimpl + authed QA (E2E_API_KEY) → a real wave, not a quick win. Best delegated to a focused agent with the blueprint + gated on `ng build` + a bundle-check that ag-grid is gone.
  - **(2) SSR/SSG of marketing routes** (`@angular/ssr` / vite-ssg → static HTML from R2): the DEEPER fix — marketing visitors get prerendered HTML + minimal client JS → TBT/TTI collapse → score 90+. Eliminates the CSR boot entirely. Bigger build-pipeline change; the durable end-state for both homepage interactivity AND the generated-site CWV.
- 🔨 LCP≤2.0s / CLS≤0.05 / INP≤200ms on a published GENERATED site too (CSR-bound, fire 7 — same template app-shell/SSG phase); Lighthouse a11y≥95 perf≥75 (homepage perf=66 today, gated on the two levers above)
