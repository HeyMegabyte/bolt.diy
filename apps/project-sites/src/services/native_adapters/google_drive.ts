...
const FindFileInput = z.object({ query: z.string().min(1).max(500), maxResults: z.number().int().min(1).max(100).default(10) });

export const googleDriveAdapter = {
  provider: 'google' as const,
  supports(action: string): boolean { return action === 'drive.find_file'; },
  requiredScopes(): string[] { return ['https://www.googleapis.com/auth/drive.metadata.readonly']; },
