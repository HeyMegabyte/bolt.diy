...
export const bluesky: Publisher = {
  ...
  async publish(_env, account, post): Promise<PublishResult> {
      ...
      record: {
        ...
        ...(post.link && {
          facets: [
            {
              index: {
                byteStart: text.indexOf(post.link),
                byteEnd: text.indexOf(post.link) + post.link.length,
              },
    ...
    if (!res.ok)
      throw new Error(`bluesky_publish_failed:${res.status}:${(await res.text()).slice(0, 200)}`);
...
export async function blueskyLogin(
  ...
  if (!res.ok)
    throw new Error(`bluesky_login_failed:${res.status}:${(await res.text()).slice(0, 200)}`);
  const data = (await res.json()) as {
    accessJwt: string;
    refreshJwt: string;
    did: string;
