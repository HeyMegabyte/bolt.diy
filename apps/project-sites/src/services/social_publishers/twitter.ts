...
export const twitter: Publisher = {
  ...
  async exchangeCode(env, { code, codeVerifier, redirectUri }) {
    ...
    if (!res.ok)
      throw new Error(`twitter_exchange_failed:${res.status}:${(await res.text()).slice(0, 200)}`);
    const data = (await res.json()) as {
      access_token: string;
      refresh_token?: string;
      expires_in?: number;
    ...
    const me = meRes.ok
      ? ((await meRes.json()) as { data?: { id: string; username: string; name: string } })
      : { data: undefined };
    return {
  ...
  async publish(env, account, post): Promise<PublishResult> {
    ...
    if (!res.ok)
      throw new Error(`twitter_publish_failed:${res.status}:${(await res.text()).slice(0, 200)}`);
    const data = (await res.json()) as { data: { id: string; text: string } };
    const handle = (account.handle ?? '').replace(/^@/, '');
    return {
      external_id: data.data.id,
      external_url: `https://x.com/${handle || 'i'}/status/${data.data.id}`,
    };
  },
