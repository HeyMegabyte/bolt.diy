...
function conf<T>(
  ...
  if (opts?.isPlaceholder) c = Math.max(0, c - 0.1);
  return {
    value,
...
function llmInferred<T>(value: T, rationale?: string): Conf<T> {
  ...
  base.confidence = Math.max(
    0,
    Math.round((base.confidence - LLM_ONLY_INFERRED_PENALTY) * 100) / 100,
  );
  return base;
}
...
function isImageRelevant(
  ...
  const typeKey = Object.keys(BUSINESS_IMAGE_KEYWORDS).find((k) =>
...
export function transformToV3(
  ...
  if (userInputs.businessPhone)
    phoneConf = mergeConf(
      phoneConf,
      conf(userInputs.businessPhone, 'user_provided', 'User provided phone'),
    );
  ...
  if (g?.website)
    websiteConf = mergeConf(websiteConf, gp(g.website, g.place_id, 'Google Places website'));
  if (!websiteConf.value) warnings.push('Missing: website URL');

  // Hours: prefer Google Places
  const rawHours = arr(p.hours);
  let hoursConf = llm(
    rawHours.map((h: Record<string, unknown>) => ({
      day: str(h.day),
      open: str(h.open),
      close: str(h.close),
      closed: !!h.closed,
  ...
  // Google identity
  const googleData =
    p.google && typeof p.google === 'object' ? (p.google as Record<string, unknown>) : {};
  let googleConf = llm(
    {
  ...
  if (g) {
    googleConf = mergeConf(
      ...
      gp(
        ...
        g.place_id,
        'Google Places verified identity',
      ),
    );
  }
  ...
  // Address
  const rawAddr =
    p.address && typeof p.address === 'object' ? (p.address as Record<string, unknown>) : {};
  let addressConf = llm(
    {
  ...
  if (userInputs.businessAddress) {
    addressConf = mergeConf(
      addressConf,
      conf(addressConf.value, 'user_provided', 'User provided address'),
    );
  }
  ...
  const bookingRaw =
    p.booking && typeof p.booking === 'object' ? (p.booking as Record<string, unknown>) : {};
  const policiesRaw =
    p.policies && typeof p.policies === 'object' ? (p.policies as Record<string, unknown>) : {};
  const accessRaw =
    p.accessibility && typeof p.accessibility === 'object'
      ? (p.accessibility as Record<string, unknown>)
      : {};
  ...
  const operations = {
    ...
    payments: llmInferred(
      strArr(p.payments),
      'LLM-inferred payment methods — unverified, may not be accurate',
    ),
    ...
    languages_spoken: llmInferred(
      strArr(p.languages_spoken),
      'LLM-inferred languages — unverified',
    ),
  };
  ...
  const services = rawServices.map((svc) => ({
    ...
    add_ons: llm(
      arr(svc.add_ons) as Array<{
        name: string;
        price_from: number | null;
        duration_minutes: number | null;
      }>,
      'LLM-suggested add-ons',
    ),
  ...
  const offerings = {
    ...
    faq: llm(
      arr(p.faq).map((f: Record<string, unknown>) => ({
  ...
  const reviewsRaw =
    p.reviews_summary && typeof p.reviews_summary === 'object'
  ...
  const trust = {
    ...
    review_platforms: llm(
      arr(s.review_platforms).map((r: Record<string, unknown>) => ({
    ...
    before_after_gallery: placeholder(
      [] as Array<{ before_url: string; after_url: string }>,
  ...
  const rawColors =
    b.colors && typeof b.colors === 'object' ? (b.colors as Record<string, unknown>) : {};
  const rawFonts =
    b.fonts && typeof b.fonts === 'object' ? (b.fonts as Record<string, unknown>) : {};
  const fallbackDesign =
    rawLogo.fallback_design && typeof rawLogo.fallback_design === 'object'
  ...
  const marketing = {
    ...
    hero_slogans: llm(
      rawSlogans.map((sl) => ({
        ...
        cta_primary:
          sl.cta_primary && typeof sl.cta_primary === 'object'
            ? (sl.cta_primary as { text: string; action: string })
            : { text: 'Get Started', action: '#contact' },
        cta_secondary:
          sl.cta_secondary && typeof sl.cta_secondary === 'object'
  ...
  const filteredHeroImages = rawHeroImages.filter((hi) =>
    isImageRelevant(
      str(hi.concept) || str(hi.alt_text) || '',
      businessType,
  ...
  const media = {
    hero_images: llm(
      filteredHeroImages.map((hi) => ({
        concept: str(hi.concept),
        url: str(hi.url),
        search_query:
          str(hi.search_query) || str(hi.search_query_stock) || str(hi.search_query_specific),
    ...
    storefront_image:
      img.storefront_image && typeof img.storefront_image === 'object'
        ? llm(
            {
    ...
    gallery:
      filteredPhotos.length > 0
        ...
        : placeholder(
            [] as Array<{ url: string; alt_text: string; source: string; license: string }>,
  ...
  const seo = {
    title: llm(
      str(rawSeo.title) || str(p.seo_title) || `${userInputs.businessName}`,
      'LLM-generated SEO title',
    ),
    description: llm(
      str(rawSeo.description) || str(p.seo_description) || '',
    ...
    neighborhood_keywords: llm(
      strArr(rawSeo.neighborhood_keywords),
      'LLM-generated local keywords',
    ),
    schema_org: llm(
      {
        ...
        aggregateRating:
          reviewsConf.value.aggregate.count > 0
            ? {
