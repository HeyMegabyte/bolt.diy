...
describe('chooseDbAllocation — the allocation LAW', () => {
  it('free plan gets no database', () => {
    ...
    expect(chooseDbAllocation({ plan: 'free', needsPostgres: true, needsIsolation: true }).dbPlan).toBe('none');
  });

  it('paid default is a per-tenant D1 database (Neon is the escape hatch)', () => {
    expect(chooseDbAllocation({ plan: 'paid' })).toEqual({ dbPlan: 'd1_tenant_db', reason: 'd1-default' });
  ...
  it('isolation / enterprise / noisy-neighbour → a dedicated Neon project', () => {
    ...
    expect(chooseDbAllocation({ plan: 'pro', noisyNeighbor: true }).dbPlan).toBe('neon_dedicated_project');
    // isolation beats a plain postgres request.
    expect(chooseDbAllocation({ plan: 'pro', needsPostgres: true, needsIsolation: true }).dbPlan).toBe(
...
describe('buildAllocation', () => {
  const dec = (d: DbAllocationDecision['dbPlan'], r: DbAllocationDecision['reason']): DbAllocationDecision => ({ dbPlan: d, reason: r });

  it('d1 + none rows carry no shard/binding/neon fields', () => {
    const rec = buildAllocation({ tenantId: 't', siteId: 's', decision: dec('d1_tenant_db', 'd1-default') });
    expect(rec).toMatchObject({ dbPlan: 'd1_tenant_db', shardId: null, hyperdriveBindingName: null, neonProjectId: null });
  });

  it('neon_shared_shard wires the shard id + the derived shard-level Hyperdrive binding', () => {
    const rec = buildAllocation({
      tenantId: 't', siteId: 's', decision: dec('neon_shared_shard', 'postgres-required'), shardIndex: 7,
      neonDatabase: 'app', neonSchema: 'public',
    });
  ...
  it('neon_dedicated_project carries the Neon project + db/schema', () => {
    const rec = buildAllocation({
      tenantId: 't', siteId: 's', decision: dec('neon_dedicated_project', 'enterprise'),
      neonProjectId: 'np-1', neonDatabase: 'app',
    });
    expect(rec).toMatchObject({ neonProjectId: 'np-1', neonDatabase: 'app', shardId: null, hyperdriveBindingName: null });
  });
});
...
describe('cacheAllocation + resolveAllocation', () => {
  ...
  it('caches by site id (+ hostname) and resolves by either', async () => {
    ...
    const rec = buildAllocation({ tenantId: 't', siteId: 's1', decision: { dbPlan: 'd1_tenant_db', reason: 'd1-default' } });
    ...
    expect(await resolveAllocation({ CACHE_KV: kv }, { siteId: 's1' })).toMatchObject({ dbPlan: 'd1_tenant_db' });
    expect(await resolveAllocation({ CACHE_KV: kv }, { hostname: 'ACME.projectsites.dev' })).toMatchObject({ siteId: 's1' });
