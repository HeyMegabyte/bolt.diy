...
const cfg: OpenFgaConfig = {
  apiUrl: 'https://authz.projectsites.dev',
  storeId: 'store1',
  authToken: 'tok',
};
...
describe('OpenFgaAuthorizationProvider', () => {
  ...
  it('check fails CLOSED on non-2xx and on network error', async () => {
    expect(
      await new OpenFgaAuthorizationProvider(
        cfg,
        fakeFetch(() => ({ status: 500 })),
      ).check({ user: 'u', relation: 'r', object: 'o' }),
    ).toBe(false);
    const throwing = (async () => {
      throw new Error('down');
    }) as unknown as typeof fetch;
    expect(
      await new OpenFgaAuthorizationProvider(cfg, throwing).check({
        user: 'u',
        relation: 'r',
        object: 'o',
      }),
  ...
  it('write + delete post tuple_keys to /write', async () => {
    ...
    expect(
      (f.calls[0].body as { writes: { tuple_keys: unknown[] } }).writes.tuple_keys,
    ).toHaveLength(1);
    expect(
      (f.calls[1].body as { deletes: { tuple_keys: unknown[] } }).deletes.tuple_keys,
    ).toHaveLength(1);
  });

  it('write throws on non-2xx', async () => {
    await expect(
      new OpenFgaAuthorizationProvider(
        cfg,
        fakeFetch(() => ({ status: 400 })),
      ).writeRelationship({ user: 'u', relation: 'r', object: 'o' }),
    ).rejects.toThrow(/write failed: 400/);
  });

  it('listObjects returns the objects array (default type site)', async () => {
    const f = fakeFetch(() => ({ status: 200, json: { objects: ['site:a', 'site:b'] } }));
    expect(
      await new OpenFgaAuthorizationProvider(cfg, f).listObjects({
        user: 'user:1',
        relation: 'can_view',
      }),
  ...
  it('batchCheck maps per-tuple results', async () => {
    const f = fakeFetch((_u, body) => ({
      status: 200,
      json: { allowed: (body as { tuple_key: { object: string } }).tuple_key.object === 'site:a' },
    }));
    const p = new OpenFgaAuthorizationProvider(cfg, f);
    expect(
      await p.batchCheck([
        { user: 'u', relation: 'r', object: 'site:a' },
        { user: 'u', relation: 'r', object: 'site:z' },
      ]),
    ).toEqual([true, false]);
  });
});
...
describe('getAuthorizationProvider OpenFGA selection', () => {
  it('returns OpenFGA when configured, else DenyAll', () => {
    expect(
      getAuthorizationProvider({ OPENFGA_API_URL: 'https://a', OPENFGA_STORE_ID: 's' } as Env),
    ).toBeInstanceOf(OpenFgaAuthorizationProvider);
