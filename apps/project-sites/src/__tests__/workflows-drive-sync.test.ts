...
jest.mock('../services/ai_context_extract.js', () => ({
  ...
  extractContext: jest
    .fn()
    .mockResolvedValue({ text: 'extracted', pages_processed: 1, truncated: false }),
}));
...
describe('DriveSyncWorkflow', () => {
  it('runs steps in documented order with 30s exponential retry config', async () => {
    ...
    const result = await wf.run(
      {
        payload: { siteId: 's1', orgId: 'o1', slug: 'acme' },
        timestamp: new Date(),
        instanceId: 'i1',
  ...
  it('is resumable: same step name returns cached value without re-invoking closure', async () => {
    ...
    const step = {
      do: async <T>(
        ...
        const actualFn = (
          typeof _configOrFn === 'function' ? (_configOrFn as () => Promise<T>) : fn
        ) as () => Promise<T>;
