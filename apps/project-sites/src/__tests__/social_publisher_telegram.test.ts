...
describe('telegram.publish', () => {
  it('sends a message and builds a public username URL', async () => {
    mock(() =>
      json({ ok: true, result: { message_id: 55, chat: { id: -1001234, username: 'mychan' } } }),
    );
    expect(await telegram.publish(ENV, account(), post())).toEqual({
      external_id: '55',
      external_url: 'https://t.me/mychan/55',
    });
  });
  it('builds a c/<id> URL when the chat has no username', async () => {
    mock(() => json({ ok: true, result: { message_id: 55, chat: { id: -1001234 } } }));
    expect((await telegram.publish(ENV, account(), post())).external_url).toBe(
      'https://t.me/c/1234/55',
    );
  });
  it('throws without a chat id', async () => {
    mock(() => json({}));
    await expect(
      telegram.publish(ENV, account({ metadata: {}, external_id: null }), post()),
    ).rejects.toThrow(/telegram_chat_id_missing/);
  });
  it('throws when the HTTP call is not OK', async () => {
    mock(() => new Response('no', { status: 400 }));
    await expect(telegram.publish(ENV, account(), post())).rejects.toThrow(
      /telegram_publish_failed:400/,
    );
  });
