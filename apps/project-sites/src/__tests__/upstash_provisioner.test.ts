...
import {
  createDatabase,
  deleteDatabase,
  MissingUpstashKeyError,
} from '../services/upstash_provisioner.js';
...
describe('auth guard', () => {
  it('throws MissingUpstashKeyError (code/service/deeplink) when email is absent', async () => {
    await expect(
      createDatabase({ UPSTASH_API_KEY: 'k' } as unknown as Env, 'db'),
    ).rejects.toBeInstanceOf(MissingUpstashKeyError);
    try {
  ...
  it('throws when key is absent (deleteDatabase too)', async () => {
    await expect(
      deleteDatabase({ UPSTASH_EMAIL: 'a@b.com' } as unknown as Env, 'db-1'),
    ).rejects.toBeInstanceOf(MissingUpstashKeyError);
  });
});
  ...
  it('truncates the database name to 32 chars', async () => {
    ...
    mockFetch((_u, init) => {
      ...
      return {
        ok: true,
        status: 200,
        json: () => Promise.resolve({ database_id: 'd', endpoint: 'e', password: 'p' }),
      };
    });
...
describe('deleteDatabase', () => {
  it('DELETEs with the URL-encoded id and Basic auth', async () => {
    ...
    expect(seenUrl).toBe(
      `https://api.upstash.com/v2/redis/database/${encodeURIComponent('db/weird id')}`,
    );
  });
