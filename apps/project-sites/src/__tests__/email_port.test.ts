...
describe('chooseEmailPath (§42)', () => {
  it('routes transactional/critical kinds to SES', () => {
    const ses: EmailKind[] = ['magic-link', 'claim-verification', 'receipt', 'billing-alert', 'security', 'domain-verification', 'transactional'];
...
describe('FakeEmailProvider', () => {
  it('accepts a valid transactional send + records it', async () => {
    ...
    const r = await p.sendTransactional({ to: 'a@b.com', subject: 'Hi', html: '<p>x</p>', kind: 'receipt', idempotencyKey: 'k1' });
  ...
  it('accepts multiple recipients', async () => {
    const p = new FakeEmailProvider();
    await p.sendTransactional({ to: ['a@b.com', 'c@d.com'], subject: 'S', html: 'h', kind: 'transactional' });
    expect(p.sent[0].to).toEqual(['a@b.com', 'c@d.com']);
  });

  it('rejects an invalid recipient / missing subject / missing html', async () => {
    const p = new FakeEmailProvider();
    await expect(p.sendTransactional({ to: 'nope', subject: 'S', html: 'h', kind: 'security' })).rejects.toBeInstanceOf(EmailInputError);
    await expect(p.sendTransactional({ to: 'a@b.com', subject: '', html: 'h', kind: 'security' })).rejects.toThrow(/subject/);
    await expect(p.sendTransactional({ to: 'a@b.com', subject: 'S', html: '', kind: 'security' })).rejects.toThrow(/html/);
