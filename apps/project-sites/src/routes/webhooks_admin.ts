...
async function gate(
  ...
  if (!(await isFlagOn(c.env, 'outbound_webhooks', { siteId, orgId })))
...
webhooksAdmin.get('/api/sites/:siteId/webhooks', async (c) => {
  ...
  return c.json({
    ok: true,
    endpoints: await listWebhookEndpoints(c.env, g.orgId, c.req.param('siteId')),
  });
});
...
webhooksAdmin.post('/api/sites/:siteId/webhooks', async (c) => {
  ...
  if (!parsed.success) {
    return c.json(
      { error: { code: 'BAD_REQUEST', message: 'Invalid endpoint', details: parsed.error.issues } },
      400,
    );
  }
  ...
  if (!res.ok) {
    return c.json(
      { error: { code: 'BAD_REQUEST', message: 'Endpoint rejected', details: res.errors } },
      400,
    );
  }
...
webhooksAdmin.delete('/api/sites/:siteId/webhooks/:id', async (c) => {
  const g = await gate(c);
  if (g instanceof Response) return g;
  const { ok } = await deleteWebhookEndpoint(
    c.env,
    g.orgId,
    c.req.param('siteId'),
    c.req.param('id'),
