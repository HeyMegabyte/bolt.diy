 ...
 */
domainPurchase.post('/api/domains/purchase', zValidator('json', purchaseSchema), async (c) => {
 ...
 */
domainPurchase.post('/api/billing/checkout/topup', zValidator('json', topupSchema), async (c) => {
  const orgId = c.get('orgId');
  if (!orgId) throw unauthorized('Must be authenticated');
  if (!walletAvailable(c.env)) {
    throw badRequest('Wallet billing not configured on this environment');
