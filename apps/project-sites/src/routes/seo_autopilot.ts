...
seoAutopilot.get('/:siteId/drafts', async (c) => {
  ...
  let sql =
    `SELECT id, site_id, org_id, route, title, description, answer_block, status,
...
seoAutopilot.post('/drafts/:draftId/approve', async (c) => {
  ...
  // Resolve the draft's site so the flag check is site-scoped.
  const owner = await dbQueryOne<{ site_id: string }>(
    c.env.DB,
    'SELECT site_id FROM seo_meta_drafts WHERE id = ? AND deleted_at IS NULL',
    [draftId],
  );
...
seoAutopilot.get('/:siteId/jsonld', async (c) => {
  ...
  if (!route) return c.json({ error: { code: 'BAD_REQUEST', message: 'route query param required' } }, 400);
