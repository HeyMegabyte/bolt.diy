...
export function checksFor(key: string): E2eCheck[] {
  ...
  return [{ label: `Homepage renders (smoke for ${key})`, kind: 'http', url: '/', expectStatus: 200 }];
}
...
async function runHttpCheck(check: E2eCheck): Promise<{ status: 'passed' | 'failed'; detail?: string }> {
  try {
    const res = await fetch(resolveCheckUrl(check), { headers: { 'User-Agent': UA, Accept: 'text/html,*/*' } });
    if (check.expectStatus && res.status !== check.expectStatus) return { status: 'failed', detail: `HTTP ${res.status}` };
    if (check.bodyIncludes) {
      const body = await res.text();
      if (!body.includes(check.bodyIncludes)) return { status: 'failed', detail: `missing "${check.bodyIncludes}"` };
    }
...
async function runBrowserCheck(env: Env, check: E2eCheck): Promise<{ status: 'passed' | 'failed'; detail?: string }> {
  ...
  try {
    ...
    try {
      ...
      if (check.textIncludes) {
        ...
        if (!text.includes(check.textIncludes)) return { status: 'failed', detail: `text "${check.textIncludes}" not found` };
      }
      return { status: 'passed' };
    } finally {
      await browser.close();
...
async function executeRun(env: Env, runId: string, checks: E2eCheck[]): Promise<void> {
  ...
  const persist = (status: RunState['status']) =>
    env.CACHE_KV.put(kvKey, JSON.stringify({ status, specs } satisfies RunState), { expirationTtl: RUN_TTL }).catch(() => {});
  await persist('running');
  await Promise.all(
    checks.map(async (ch, i) => {
      const t0 = Date.now();
      const r = ch.kind === 'browser' ? await runBrowserCheck(env, ch) : await runHttpCheck(ch);
      specs[i] = { path: ch.label, status: r.status, durationMs: Date.now() - t0, detail: r.detail };
      await persist('running');
    }),
  );
...
featureE2e.post('/api/feature-e2e/:key/run', async (c) => {
  ...
  const initial: RunState = { status: 'running', specs: checks.map((ch) => ({ path: ch.label, status: 'queued' })) };
