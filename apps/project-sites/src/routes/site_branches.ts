...
siteBranchesApp.post('/api/sites/:siteId/branches/:branchId/review', async (c) => {
  ...
  const slackUrl = (c.env as unknown as Record<string, string | undefined>)['BRANCHES_SLACK_WEBHOOK'];
  const branch = await requestReview(c.env.DB, branchId, slackUrl);
...
siteBranchesApp.post('/api/sites/:siteId/branches/:branchId/approve', async (c) => {
  ...
  const { userId } = await assertOwner(c, siteId);
  const branchId = c.req.param('branchId');
  const result = await approveBranch(c.env.DB, branchId, userId);
...
siteBranchesApp.post(
  ...
  async (c) => {
    ...
    const branch = await mergeBranch(c.env.DB, branchId, build_version);
...
siteBranchesApp.post('/api/sites/:siteId/branches/:branchId/close', async (c) => {
  ...
  await assertOwner(c, siteId);
  const branchId = c.req.param('branchId');
  const branch = await closeBranch(c.env.DB, branchId);
