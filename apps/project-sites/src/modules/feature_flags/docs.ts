...
export const FLAG_DOCS: Record<string, FlagDocs> = {
  ...
  sparkline_overlays: {
    explanation:
      "Every numeric metric in the admin gets a tiny SVG sparkline beside it showing the 7-day trend. Inline, ~22px tall, signal-driven. Tabular-num font + tabular-nums CSS so digits don't jitter. Hover reveals exact values + delta vs prior period. Compounds with #20 compare-with-last-period.",
    smoke_test: [
      "Enable flag at /admin/feature-flags → set to 100% rollout",
      "Visit /admin → sparklines appear next to 'New sites this week: 12' and 'Lead conversions: 4.2%'",
      "Inspect DOM: <app-sparkline> with data-upgrade='r2-1' attribute; SVG polyline + circle end-marker",
    ],
  },
  split_view_drawer: {
    explanation:
      "Two-pane view: list on left, detail on right. Any list row dispatches `ps-split-open` CustomEvent with {title, subtitle, route, bodyHtml}; drawer slides in from right without navigation. 'Open full' link uses Angular router for SPA-smooth full nav. Per [[verification-loop]] no page reloads.",
    smoke_test: [
      "Enable flag → reload /admin",
      "Click 'Open split-view demo' button → drawer slides in from right with Bayonne Bakery detail",
      "Click 'Open full →' link → router navigates to /admin/sites without reload",
      "Click × → drawer closes; list still visible",
    ],
  },
  row_hover_actions: {
    explanation:
      "Hover any list row tagged with [appRowActions] → action toolbar (publish/archive/duplicate/share) injected absolutely-positioned at the row's right edge. Mouse leave removes. Each action button fires the row's `onClick` callback. Zero re-render of the parent list (pure DOM mutation).",
    smoke_test: [
      "Enable flag → reload /admin",
      "Find 'Hover this row → quick actions appear' bar in the upgrades shell",
      "Hover the row → 4 action buttons appear (Publish / Archive / Duplicate / Share)",
      "Click an action → tracked in recent-actions; mouse leave removes the toolbar",
    ],
  },
  saved_views: {
    explanation:
      "Press Cmd+Shift+S on any admin route → prompts for a label → saves {label, route, search, createdAt} to localStorage (+ D1 sync in production). Renders in the 'Saved views' section of the upgrades shell; click any saved view → router navigates with the original query params restored.",
    smoke_test: [
      "Enable flag → visit /admin",
      "Press Cmd+Shift+S → name prompt appears",
      "Enter a label → view saved",
      "'Saved views' section now shows the entry; click to navigate; × deletes",
    ],
  },
  predicted_actions: {
    explanation:
      "Heuristic-based 'next action' suggestions panel. Weekday morning → 'Open billing for last month'. Recently viewed exists → 'Reopen X'. Production uses Workers AI free Llama to classify the user's last 90 days of admin actions + emit confidence-ranked predictions. Confidence shown as high/med/low color band.",
    smoke_test: [
      "Enable flag → visit /admin",
      "Panel appears below topbar with ✨ icon + 'Predicted next' header",
      "1-4 prediction cards rendered with label / reason / confidence % chip",
      "Click any card → navigates via Angular router (no reload)",
    ],
  },
  // ── 30 big-bet features (signature surfaces, 4-20 wks each)
  visual_editor_drag_drop: { explanation: 'Webflow-class drag-drop site builder beyond bolt.diy. Components panel, layout grid, breakpoint editor, per-element style editor. Lets non-technical users design without prompts. 12-16 wks of dev; flag-gated so we ship + iterate per-customer.', smoke_test: ['POST /api/visual-editor/save with body {site_id, layout, breakpoint}', 'Server persists layout JSON + returns saved_at', 'Multiple breakpoints (desktop/tablet/mobile) saved separately'] },
  ecommerce_engine: { explanation: 'Medusa.js-backed e-commerce: products + cart + checkout + inventory + orders + returns. Square or Stripe per [[payments-routing]]. Per [[ecommerce-stack]] this is the canonical engine.', smoke_test: ['GET /api/ecommerce/products/{siteId} → returns array of products with price/sku/inventory', 'POST /api/ecommerce/orders → creates order + returns Stripe Checkout URL'] },
  native_booking_engine: { explanation: 'Cal.com-class booking per customer site. Availability rules, group bookings, deposits via Stripe, no-show fees, calendar sync (Google/Apple/Outlook), SMS reminders.', smoke_test: ['GET /api/booking/slots/{siteId} → 5 demo slots in the next 5 hours', 'POST /api/booking/reserve with body {slot_id, email} → confirmation + email sent'] },
  lms_engine: { explanation: 'Learning Management System per site: modules, lessons (text/video/quiz), drip release, certificates, student progress tracking, Stripe-paid courses, completion certificates.', smoke_test: ['POST /api/lms/courses with body {site_id, title, modules:[]} → returns id + status:draft', 'GET /api/lms/courses/{siteId} → lists courses'] },
  community_engine: { explanation: 'Discourse-class forum: threaded topics, replies, moderation tools, badges, pinned/locked topics, AI-assisted moderation via Workers AI.', smoke_test: ['POST /api/community/topics with body {site_id, title, body, author_email}', 'GET /api/community/topics/{siteId} → list with reply counts + pinned/locked status'] },
  newsletter_engine: { explanation: 'Listmonk-class email engine: campaigns + drip sequences + segmentation + double opt-in + A/B subject testing. Sends via Resend SMTP relay. Compliance: GDPR + CAN-SPAM + double opt-in mandatory.', smoke_test: ['POST /api/newsletter/campaigns → returns id + estimated_recipients', 'POST /api/newsletter/subscribe → confirm email sent (double opt-in)'] },
  membership_paywall: { explanation: 'Tier-based membership with Stripe billing. Per-page access gating (free / bronze / silver / gold). Discord/Slack auto-grant. Drip content unlock by tenure.', smoke_test: ['POST /api/membership/tiers → creates tier with stripe_price_id', 'GET /api/membership/tiers/{siteId} → 3 default tiers (Bronze $5, Silver $15, Gold $50)'] },
  donations_engine: { explanation: 'Donorbox-class for nonprofits: one-time + recurring + DAFpay (Chariot/Fidelity DAF routing) + memorial gifts + donor wall + employer matching lookup (Double the Donation API).', smoke_test: ['POST /api/donations/campaigns with body {site_id, name, goal_cents}', 'POST /api/donations/process with body {campaign_id, email, amount_cents, recurring} → tax_receipt_sent:true + dafpay_url returned'] },
  native_mobile_admin: { explanation: 'Capacitor 6 wrapper of admin SPA → iOS + Android apps. Push notifications (deploy/lead/billing), biometric auth, native share, deep links, App Store + Play Store submission.', smoke_test: ['POST /api/mobile/register with body {user_id, platform, device_id, push_token, app_version}', 'Returns store_url + push_enabled:true'] },
  native_desktop_admin: { explanation: 'Tauri 2 native app for macOS/Windows/Linux: menu-bar icon, system notifications, file watcher integration, deep linking. ~50MB binary vs 200MB Electron.', smoke_test: ['GET /api/desktop/info → returns download URLs + SHA256 for all 3 platforms'] },
  browser_extension: { explanation: 'Chrome/Firefox/Edge extension: edit any element on a customer site from any page, quick deploy from any tab, inline AI prompt overlay, inspect site metrics directly on the rendered page.', smoke_test: ['GET /api/extension/info → returns store URLs + version + permissions + feature list'] },
  chat_ops_bot: { explanation: 'Slack/Teams/Discord bot. Slash commands: /deploy, /incidents, /leads, /billing. Real-time pings to channels for deploy events, lead captures, SLO breaches. OAuth-based install.', smoke_test: ['POST /api/chatops/connect with body {org_id, platform:slack, webhook_url}', 'Returns slash_commands + status:connected'] },
  soc2_program: { explanation: 'SOC 2 Type II audit program. Controls catalog (CC1-CC9, A1, C1, P1), continuous evidence collection from D1 + audit_chain, auditor portal, Drata/Vanta integration ready. Enables enterprise sales contracts.', smoke_test: ['GET /api/soc2/controls → 9-control overview with status (planned/in_progress/operating/tested/passed)', 'Each control shows family + description + status'] },
  hipaa_variant: { explanation: 'HIPAA-compliant infrastructure variant. BAA program (signed contracts in D1), encrypted-at-rest data (already in place via ai_crypto), 6-year audit retention, breach notification within 72h, segmented isolated D1.', smoke_test: ['POST /api/hipaa/baa with body {customer_org_id, business_name}', 'Returns signed_at + pdf_url + control list'] },
  pci_dss_l1: { explanation: 'PCI DSS Level 1 path. Tokenization service (cards → tokens stored in D1, never raw PAN). Scope-reduction architecture so customer sites can take cards without inheriting PCI scope.', smoke_test: ['POST /api/pci/tokenize with body {customer_id, last4, brand}', 'Returns token (tok_*) — last4 + brand stored, raw PAN never persisted'] },
  enterprise_sso: { explanation: 'Native SAML 2.0 + OIDC IdP federation. Customers connect Okta / Azure AD / Auth0 / Google Workspace directly (not just Clerk). Sells the enterprise tier; required by every 1000+ employee org.', smoke_test: ['POST /api/sso/connect with body {org_id, protocol:saml, idp_metadata_url, idp_entity_id}', 'Returns sp_entity_id + acs_url + slo_url + status:pending'] },
  d1_multi_region: { explanation: 'Multi-region D1 active-active replication via Sessions API. Cross-region bookmark pinning, automatic failover on primary outage, per-replica health monitoring. Existing CF Sessions API.', smoke_test: ['GET /api/d1/replication-status', 'Returns primary_region + 4 replicas with lag_ms + healthy flag', 'sessions_api_enabled:true'] },
  byo_cloudflare: { explanation: 'Enterprise tenants connect their own Cloudflare account via OAuth. We orchestrate (deploy Worker, manage D1), they own the resources. Eliminates "vendor lock" objection for enterprise sales.', smoke_test: ['POST /api/byo-cloudflare/connect with body {org_id, cf_account_id}', 'Returns oauth_url + 5 required scopes (account/workers/d1/r2/dns)'] },
  worker_marketplace: { explanation: 'Customers publish Worker scripts as installable adapters that other customers install. 70/30 revenue split via Stripe Connect. Examples: Stripe Connect Express adapter, Twilio SMS notifier, OpenAI GPT-5 chat proxy.', smoke_test: ['GET /api/worker-marketplace → 3 demo listings with install_count + price + rating'] },
  domain_reseller: { explanation: 'Sell domains directly via OpenSRS API. One-click search + buy + auto-wire DNS to customer site. ~$5-15 margin per domain. Bundles with [[secret-auto-provisioning]] Tier-4 manual setup.', smoke_test: ['GET /api/domain-reseller/search?q=bayonnebakery', 'Returns 5 suggestions across TLDs with available/price/registrar'] },
  brand_voice_clone: { explanation: 'Per-brand ElevenLabs voice clone. Customer uploads 5-10 minutes of audio with signed consent. Trained voice powers the per-page podcast (#44 page_podcast), IVR voice agent, audio tour narration.', smoke_test: ['POST /api/voice-clones with body {org_id, name, sample_r2_keys:[]}', 'Returns elevenlabs_voice_id + status:training + consent_required:true'] },
  ai_agent_marketplace: { explanation: 'Customers build AI agents using projectsites MCP tools, publish as installable agents, earn 70/30 revenue split. Examples: booking assistant, restaurant menu Q&A, lead qualifier pro.', smoke_test: ['GET /api/ai-agent-marketplace → 3 demo listings with mcp_tools list + install_count + rating'] },
  customer_site_copilot: { explanation: 'Embeddable AI chat widget on customer sites. RAG-grounded on the site\'s own content via Vectorize 768-dim. Answers visitor questions from the customer\'s research_data + page content. Compounds with #11 site_mcp_server.', smoke_test: ['POST /api/site-copilot/index/{siteId} → triggers Vectorize indexing', 'Returns doc_count + namespace + status:indexed + monthly cost estimate'] },
  ai_video_courses: { explanation: 'Outline → multi-lesson video course via Veo + ElevenLabs. Auto-stitches 7-8s Veo clips into 8-min lessons with brand-cloned narration. Course goes straight into the LMS engine (#4).', smoke_test: ['POST /api/ai-video-courses with body {site_id, title, outline}', 'Returns lesson_count derived from outline numbered list + cost_usd + status:generating'] },
  ai_ab_test_generator: { explanation: 'Admin sets goal ("increase booking conversions"); AI generates 3-5 variants of the targeted section; Thompson sampling routes traffic; auto-promotes winner at significance. Compounds with #8 per_page_ab_variants.', smoke_test: ['POST /api/ai-ab-experiments with body {site_id, goal, variant_count}', 'Returns variants array with traffic_split + sampling:thompson + auto_promote:true'] },
  sms_marketing: { explanation: 'Twilio-backed SMS campaigns. Drip sequences, segmentation, STOP-keyword auto opt-out (TCPA compliant), 10DLC registration, scheduled sends. Per [[copy-writing]] anti-slop applies.', smoke_test: ['POST /api/sms-campaigns with body {site_id, name, body, segment}', 'Returns estimated_recipients + cost_estimate_usd ($0.0075 per recipient)'] },
  affiliate_program: { explanation: 'Per-customer affiliate portal: tracking links, conversion attribution, commission tiers (default 20%), Stripe Connect payouts. Customer\'s affiliates promote their site; we facilitate.', smoke_test: ['POST /api/affiliates with body {org_id, email, commission_pct}', 'Returns code (uppercase 12-char) + referral_link + stripe_connect_required:true'] },
  loyalty_engine: { explanation: 'Points + tiers (bronze/silver/gold/platinum) + redemption catalog. Auto-tier on purchase events. Birthday bonuses, referral multipliers, anniversary rewards. Compounds with #2 ecommerce_engine.', smoke_test: ['POST /api/loyalty/programs with body {site_id, name, points_per_dollar}', 'Returns 4-tier structure with min_points + perks per tier'] },
  crm_engine: { explanation: 'HubSpot-class CRM per customer site. Deal pipeline (lead → qualified → proposal → negotiation → won/lost), tasks, calls, emails, calendar integration, AI-summarized email threads. Replaces external CRM for SMB customers.', smoke_test: ['GET /api/crm/deals/{siteId} → 3 demo deals at different stages with owner + next_step + value', 'POST /api/crm/deals → creates deal at stage:lead'] },
  cdp_engine: { explanation: 'Customer Data Platform: unified identity across web + SMS + email + CRM + sales + support. Identity resolution (email+phone → unified_id). Real-time event stream. Powers segmentation for newsletter/SMS/loyalty.', smoke_test: ['POST /api/cdp/profiles with body {site_id, email, phone, traits}', 'Returns unified_id + identity_resolved:true when email+phone both present', 'POST /api/cdp/events with body {profile_id, source, kind, payload} → ts returned'] },
  ide_sandbox: { explanation: "Cloudflare Sandbox-backed full IDE for power users (vs. bolt.diy's chat-only editor). Monaco editor + terminal + file tree + live preview. Each session is its own ephemeral Sandbox DO with node:22-slim container. 30-min idle auto-destroy. Different from the editor — opt-in for users who want to write code directly.", smoke_test: ['POST /api/ide-sandbox/spin-up with body {site_id, user_id}', 'Returns sandbox_id + ide_url + monaco_url + terminal_url + preview_url + estimated_boot_ms (~800ms)', 'GET /api/ide-sandbox/status/{sandboxId} → state + age_seconds + idle_seconds + auto_destroy_in_seconds', 'POST /api/ide-sandbox/destroy/{sandboxId} → state:destroyed'] },
  multi_agent_concurrent: { explanation: "Multiple specialist agents work CONCURRENTLY on the same site. Roster: design / copy / seo / a11y / media / motion / qa. Each gets its own context window + file partition. Real-time stream events fan into admin UI. Per [[monitor-orchestration]] — production: actual Agent tool calls via Anthropic API; mock here returns realistic shape.", smoke_test: ['POST /api/multi-agent/start with body {site_id, agents:["design","copy","seo","a11y"], prompt}', 'Returns run_id + agents[] queued + stream_url + estimated_total_ms', 'GET /api/multi-agent/runs/{siteId} → run history', 'GET /api/multi-agent/run/{runId} → live agent status + last 50 stream events'] },
  progressive_skeleton_build: { explanation: 'Site goes LIVE as a skeleton (CSS-grid placeholders with shimmer) the moment publish is requested. Real components stream in via SSE as the AI generates them, swapping skeleton blocks via View Transitions API. Visitors see something IMMEDIATELY instead of a 5-min blank page during full generation.', smoke_test: ['POST /api/progressive-build/publish-skeleton/{siteId} → returns skeleton_url + 9 component slots + stream_endpoint', 'GET /api/progressive-build/stream/{siteId} → progress_pct + components_done + components_remaining + next_component'] },
  // ── Wave 2C — Swarm/Stream/Marketplace/DNA
  swarm_editor: {
