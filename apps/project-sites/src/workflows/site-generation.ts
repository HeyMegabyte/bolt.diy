...
export function buildPrompt(params: SiteGenerationParams): string {
  ...
  return [
    ...
    '   - Fonts (DO NOT SKIP — the #1 reason themes look generic): the ADDITIONAL CONTEXT below carries a "Fonts (MANDATORY)" block with the EXACT Google Fonts `<link href>` and the EXACT `--font-heading` / `--font-body` CSS stacks. Add that `<link>` to `<head>` and set those two CSS variables, then make headings (h1–h6, nav, hero) use `var(--font-heading)` and body use `var(--font-body)`. A build whose headings compute to system-ui / -apple-system has NOT applied the theme — always wire the real fonts, never leave the template default.',
