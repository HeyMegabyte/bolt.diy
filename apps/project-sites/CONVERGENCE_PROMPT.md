...
- **Each monitoring fire:** (a) cheap liveness + CHANGE-DETECTION — `git fetch` (worker/code change?) + `/api/health` 200 + `curl /api/network-analytics` populated + fetch `/` and extract the `main-<hash>.js` bundle hash. The bundle hash is the FRONTEND change-detector (a concurrent-session R2 deploy is invisible to git-sync): if the hash CHANGED since the last fire, a real-browser render regression is possible → escalate to ONE `verify-analytics-fix.mjs` / reconciler run; if UNCHANGED and git is in-sync, prod is byte-identical to the last green pass → the curl heartbeat suffices, a re-sweep would be padding. Baseline as of 2026-08-09 (HEAD `3c571046`): `main-N276EO52.js`. NOT a full Browserbase re-sweep every fire. (b) advance the real `[ ]` backlog ONLY where genuinely autonomous-bounded (currently ~none — the wire-me services are removed/premature/redundant/against-doctrine; ag-grid→TanStack is a dedicated session; infra + pricing are Brian-gated); (c) report honestly + surface the human-held blockers.
