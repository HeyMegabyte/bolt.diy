...
describe('FlagGateNoticeComponent (shared cockpit flag-gate primitive)', () => {
  ...
  it('surfaces the feature, the flag in <code>, and a SPA link to Feature Flags', () => {
    const el = build();
    const gate = el.querySelector('[data-testid="marketplace-flag-gate"]')!;
    ...
    expect(link?.textContent?.replace(/\s+/g, ' ').trim()).withContext('label matches the renamed section').toBe('Feature Flags');
  });
  ...
  it('omits the heading + suffix by default', () => {
    ...
    expect(body.endsWith('Feature Flags.')).withContext('sentence ends at "Feature Flags." with no suffix').toBeTrue();
  });
  ...
  it('renders an optional bold heading + a trailing suffix clause when provided', () => {
    ...
    const body = el.querySelector('.flag-gate__body')?.textContent?.replace(/\s+/g, ' ').trim() ?? '';
    expect(body).toContain('Enable it in Feature Flags to search Worker tail logs and view cost-by-route.');
  });

  // Owner-aware messaging: the operator-only "Enable it in Feature Flags" CTA is
  ...
  it('OPERATOR identity → keeps the "Enable it in Feature Flags" CTA link', () => {
  ...
  it('OWNER identity → plain plan-aware copy, NO operator Feature Flags link', () => {
    ...
    expect(body).not.toContain('Feature Flags');
