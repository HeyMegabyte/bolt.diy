 ...
 * partial / all-fail) so the user knows exactly what landed. overrideComponent
 * strips the template; uploadFiles() is driven directly with controlled
 * per-file results.
 */
...
function make(): { c: GlobalDropZoneComponent; post: jasmine.Spy; toast: ToastSpies } {
  ...
  return { c: TestBed.createComponent(GlobalDropZoneComponent).componentInstance, post, toast };
}
...
describe('GlobalDropZoneComponent (upload resilience)', () => {
  ...
  it('all uploads succeed → success toast', async () => {
  ...
  it('all uploads fail → error toast (nothing landed)', async () => {
    const { c, post, toast } = make();
