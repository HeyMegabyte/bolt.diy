...
export const APPS_CATALOG: ReadonlyArray<CatalogApp> = [
  ...
  },
  {
    ...
    env: [
      ...
      { key: 'DATABASE_URL', description: 'Postgres connection string (Neon)', required: true, auto: 'postgres_url' },
      { key: 'REDIS_URL', description: 'Redis connection URL (Upstash)', required: true, auto: 'redis_url' },
      { key: 'JWT_SECRET', description: '32-byte JWT signing secret', required: true, auto: 'secret' },
      { key: 'FRONTEND_URL', description: 'Public canonical URL (https://social.projectsites.dev)', required: true, auto: 'public_url' },
      ...
      // Production-required (service won't function correctly without)
      { key: 'IS_GENERAL', description: 'MUST be "true" on self-host — fixes frontend routing', required: true, default: 'true' },
      { key: 'RUN_CRON', description: 'MUST be set on the Temporal worker process', required: true, default: 'true' },
      { key: 'DISABLE_REGISTRATION', description: 'Lock signup after admin created', required: true, default: 'true' },
      { key: 'TEMPORAL_ADDRESS', description: 'Temporal Cloud host:port', required: true, default: 'social-projectsites.g3erb.tmprl.cloud:7233' },
      { key: 'TEMPORAL_NAMESPACE', description: 'Temporal Cloud namespace', required: true, default: 'social-projectsites.g3erb' },
      { key: 'TEMPORAL_TLS', description: 'Enable TLS for Temporal Cloud', required: true, default: 'true' },
      // Storage (R2)
      { key: 'STORAGE_PROVIDER', description: 'Must be "cloudflare" for R2', required: true, default: 'cloudflare' },
      { key: 'CLOUDFLARE_ACCOUNT_ID', description: 'CF Account ID for R2', required: true, default: '84fa0d1b16ff8086dd958c468ce7fd59' },
      { key: 'CLOUDFLARE_ACCESS_KEY', description: 'R2 Access Key ID', required: true, auto: 'secret' },
      { key: 'CLOUDFLARE_SECRET_ACCESS_KEY', description: 'R2 Secret Access Key', required: true, auto: 'secret' },
      { key: 'CLOUDFLARE_BUCKETNAME', description: 'R2 bucket name', required: true, default: 'social-media-assets' },
      { key: 'CLOUDFLARE_BUCKET_URL', description: 'Public R2 URL', required: true, default: 'https://pub-4157dd570f0641458b9d4fabad0749d4.r2.dev' },
      { key: 'CLOUDFLARE_REGION', description: 'R2 region', required: true, default: 'auto' },
      // Email (Resend)
      { key: 'EMAIL_PROVIDER', description: 'Email provider', required: false, default: 'resend' },
      { key: 'RESEND_API_KEY', description: 'Resend API key', required: false, auto: 'secret' },
      { key: 'EMAIL_FROM_ADDRESS', description: 'From address', required: false, default: 'social@megabyte.space' },
      { key: 'EMAIL_FROM_NAME', description: 'From name', required: false, default: 'ProjectSites Social' },
      // Temporal Cloud (replaces Docker-based BullMQ — REQUIRED for scheduled posts)
      { key: 'TEMPORAL_ADDRESS', description: 'Temporal Cloud host:port', required: true, default: 'social-projectsites.g3erb.tmprl.cloud:7233' },
      { key: 'TEMPORAL_NAMESPACE', description: 'Temporal Cloud namespace', required: true, default: 'social-projectsites.g3erb' },
      { key: 'TEMPORAL_TLS', description: 'Enable TLS for Temporal Cloud gRPC', required: true, default: 'true' },
      { key: 'TEMPORAL_API_KEY', description: 'Temporal Cloud API key', required: true, auto: 'secret' },
      // Short links (Dub — platform standard, replaces Kutt.it from reference deploy)
      { key: 'DUB_TOKEN', description: 'Dub.co API token for short links', required: false, auto: 'secret' },
      { key: 'DUB_SHORT_LINK_DOMAIN', description: 'Dub short link domain', required: false, default: 'claimyour.site' },
      // Platform OAuth — keys from reference deploy (postiz.megabyte.space), carry forward
      { key: 'X_API_KEY', description: 'X (Twitter) API key', required: false, auto: 'secret' },
      { key: 'X_API_SECRET', description: 'X (Twitter) API secret', required: false, auto: 'secret' },
      { key: 'LINKEDIN_CLIENT_ID', description: 'LinkedIn OAuth client ID', required: false, auto: 'secret' },
      { key: 'LINKEDIN_CLIENT_SECRET', description: 'LinkedIn OAuth client secret', required: false, auto: 'secret' },
      { key: 'FACEBOOK_APP_ID', description: 'Facebook App ID', required: false, auto: 'secret' },
      { key: 'FACEBOOK_APP_SECRET', description: 'Facebook App secret', required: false, auto: 'secret' },
      { key: 'YOUTUBE_CLIENT_ID', description: 'YouTube/Google OAuth client ID', required: false, auto: 'secret' },
      { key: 'YOUTUBE_CLIENT_SECRET', description: 'YouTube/Google OAuth client secret', required: false, auto: 'secret' },
      { key: 'TIKTOK_CLIENT_ID', description: 'TikTok client ID', required: false, auto: 'secret' },
      { key: 'TIKTOK_CLIENT_SECRET', description: 'TikTok client secret', required: false, auto: 'secret' },
      { key: 'REDDIT_CLIENT_ID', description: 'Reddit API client ID', required: false, auto: 'secret' },
      { key: 'REDDIT_CLIENT_SECRET', description: 'Reddit API client secret', required: false, auto: 'secret' },
      { key: 'PINTEREST_CLIENT_ID', description: 'Pinterest client ID', required: false, auto: 'secret' },
      { key: 'PINTEREST_CLIENT_SECRET', description: 'Pinterest client secret', required: false, auto: 'secret' },
      { key: 'DISCORD_CLIENT_ID', description: 'Discord OAuth client ID', required: false, auto: 'secret' },
      { key: 'DISCORD_CLIENT_SECRET', description: 'Discord OAuth client secret', required: false, auto: 'secret' },
      { key: 'DISCORD_BOT_TOKEN_ID', description: 'Discord bot token', required: false, auto: 'secret' },
      { key: 'SLACK_ID', description: 'Slack app ID', required: false, auto: 'secret' },
      { key: 'SLACK_SECRET', description: 'Slack app secret', required: false, auto: 'secret' },
      { key: 'SLACK_SIGNING_SECRET', description: 'Slack signing secret', required: false, auto: 'secret' },
      { key: 'DRIBBBLE_CLIENT_ID', description: 'Dribbble OAuth client ID', required: false, auto: 'secret' },
      { key: 'DRIBBBLE_CLIENT_SECRET', description: 'Dribbble OAuth client secret', required: false, auto: 'secret' },
      { key: 'GITHUB_CLIENT_ID', description: 'GitHub OAuth client ID (sign-in only)', required: false, auto: 'secret' },
      { key: 'GITHUB_CLIENT_SECRET', description: 'GitHub OAuth client secret', required: false, auto: 'secret' },
      { key: 'BEEHIIVE_API_KEY', description: 'Beehiiv newsletter API key', required: false, auto: 'secret' },
      { key: 'BEEHIIVE_PUBLICATION_ID', description: 'Beehiiv publication ID', required: false, auto: 'secret' },
      { key: 'TELEGRAM_TOKEN', description: 'Telegram bot token', required: false, auto: 'secret' },
      { key: 'TELEGRAM_BOT_NAME', description: 'Telegram bot display name', required: false },
      // Observability
      { key: 'NEXT_PUBLIC_POSTHOG_KEY', description: 'PostHog project API key', required: false },
      { key: 'NEXT_PUBLIC_POSTHOG_HOST', description: 'PostHog ingestion host', required: false, default: 'https://us.i.posthog.com' },
      { key: 'NEXT_PUBLIC_SENTRY_DSN', description: 'Sentry DSN (dedicated postiz project)', required: false, auto: 'secret' },
    ],
    memoryMB: 1024,
    estCostMonthly: 24,
