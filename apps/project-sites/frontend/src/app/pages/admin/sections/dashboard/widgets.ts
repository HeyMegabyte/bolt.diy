...
// ─── Inbox Metrics (Pulse Inbox analytics) ──────────────────────────────
...
@Component({
  ...
  template: `
    <div class="wrap" appReveal>
      <header class="hd">
        <span class="t">Inbox metrics</span>
        <span class="win">last {{ windowDays() }}d</span>
      </header>

      <div class="kpis">
        <div class="kpi" appReveal>
          <div class="lbl">First response</div>
          <div class="val">{{ humanDuration(firstResponse()) }}</div>
        </div>
        <div class="kpi" appReveal>
          <div class="lbl">Resolution time</div>
          <div class="val">{{ humanDuration(resolution()) }}</div>
        </div>
        <div class="kpi" appReveal>
          <div class="lbl">CSAT</div>
          <div class="val">
            <app-rolling-counter [value]="csat()" [decimals]="2" suffix=" / 5" />
          </div>
        </div>
        <div class="kpi" appReveal>
          <div class="lbl">Today</div>
          <div class="val">
            <app-rolling-counter [value]="todayCount()" />
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="card">
          <div class="card-t">Sentiment</div>
          <svg viewBox="0 0 120 120" class="donut" aria-hidden="true">
            @for (s of sentimentSegs(); track s.label) {
              <circle
                cx="60" cy="60" r="48"
                fill="none"
                [attr.stroke]="s.color"
        ...
        <div class="card">
          <div class="card-t">Intent</div>
          <ul class="intent">
            @for (i of intentRows(); track i.label) {
              <li appReveal>
                <span class="lg-label">{{ i.label }}</span>
                <div class="bar-fill">
                  <div class="bar-inner" [style.width.%]="i.pct" [attr.data-tone]="i.label"></div>
                </div>
                <span class="lg-n">{{ i.value }}</span>
              </li>
            }
            @if (intentRows().length === 0) {
              <li class="empty">No tagged conversations yet.</li>
            }
          </ul>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-t">Conversations per day</div>
        <svg viewBox="0 0 600 100" class="spark" preserveAspectRatio="none" aria-hidden="true">
          <path [attr.d]="sparkPath()" fill="none" stroke="var(--ps-accent, #00e5ff)" stroke-width="2" />
          <path [attr.d]="sparkArea()" fill="var(--ps-accent, #00e5ff)" opacity="0.12" />
        </svg>
      </div>
    </div>
  `,
  styles: [
    `
      ...
      .kpi {
        ...
        background: linear-gradient(135deg, rgba(0, 229, 255, 0.05), rgba(124, 58, 237, 0.04));
        border: 1px solid rgba(0, 229, 255, 0.1);
      }
      .lbl { font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.6; }
      .val {
        font-family: 'Sora', system-ui, sans-serif; font-size: 1.4rem;
        font-weight: 600; color: var(--ps-ink, #f4f4ff); margin-top: 4px;
      }
      .row {
      ...
      .card-t { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.6; margin-bottom: 8px; }
      .donut { width: 110px; height: 110px; display: block; margin: 0 auto 8px; }
      .legend { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 4px; }
      .legend li {
        display: flex; align-items: center; gap: 8px;
        font-size: 0.8rem; color: var(--ps-ink, #f4f4ff);
      }
      .legend i { width: 8px; height: 8px; border-radius: 2px; display: inline-block; }
      .lg-label { flex: 1; text-transform: capitalize; }
      .lg-n { font-family: 'JetBrains Mono', monospace; font-size: 0.78rem; opacity: 0.75; }
      .intent { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 6px; }
      .intent li { display: grid; grid-template-columns: 80px 1fr 40px; gap: 8px; align-items: center; font-size: 0.8rem; }
      .bar-fill { height: 6px; border-radius: 3px; background: rgba(0, 229, 255, 0.08); overflow: hidden; }
      .bar-inner { height: 100%; background: var(--ps-accent, #00e5ff); transition: width 500ms ease; }
      [data-tone='complaint'] { background: #f87171; }
      [data-tone='billing']   { background: #fbbf24; }
      [data-tone='sales']     { background: #34d399; }
      [data-tone='support']   { background: #60a5fa; }
      .empty { opacity: 0.55; font-style: italic; }
      .spark { width: 100%; height: 100px; }
    `,
  ],
})
export class WInboxMetricsComponent {
  @Input() set props(v: InboxMetricsProps | undefined) {
    this.windowDays.set(v?.window_days ?? 30);
    this.firstResponse.set(v?.avg_first_response_seconds ?? 0);
    this.resolution.set(v?.avg_resolution_seconds ?? 0);
    this.csat.set(v?.csat_avg ?? 0);
    const cpd = v?.conversations_per_day ?? [];
    this.daily.set(cpd);
    this.todayCount.set(cpd[cpd.length - 1]?.count ?? 0);
    this.sentiment.set(v?.sentiment_breakdown ?? { positive: 0, neutral: 0, negative: 0 });
    this.intent.set(v?.intent_breakdown ?? {});
  }
  windowDays = signal(30);
  firstResponse = signal(0);
  resolution = signal(0);
  csat = signal(0);
  todayCount = signal(0);
  daily = signal<{ date: string; count: number }[]>([]);
  sentiment = signal<{ positive: number; neutral: number; negative: number }>({
    positive: 0,
    neutral: 0,
    negative: 0,
  });
  intent = signal<Record<string, number>>({});

  humanDuration(secs: number): string {
  ...
  sentimentSegs = computed(() => {
    const s = this.sentiment();
    const total = s.positive + s.neutral + s.negative || 1;
    const C = 2 * Math.PI * 48;
    const rows = [
      { label: 'positive', value: s.positive, color: '#34d399' },
      { label: 'neutral', value: s.neutral, color: '#60a5fa' },
      { label: 'negative', value: s.negative, color: '#f87171' },
    ];
    let acc = 0;
    return rows.map((r) => {
  ...
  sparkPath = computed(() => {
    ...
    const step = 600 / Math.max(1, data.length - 1);
    return data
      .map((d, i) => `${i ? 'L' : 'M'} ${(i * step).toFixed(1)} ${(100 - (d.count / max) * 100).toFixed(1)}`)
      .join(' ');
  });
  ...
  sparkArea = computed(() => {
    ...
    if (!path) return '';
    return `${path} L 600 100 L 0 100 Z`;
  });
}
...
@Component({
  ...
  template: `
    @switch (name()) {
      ...
      @case ('inbox-metrics')      { <app-w-inbox-metrics [props]="props()" /> }
