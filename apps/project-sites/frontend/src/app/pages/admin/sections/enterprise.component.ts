...
export class AdminEnterpriseComponent {
  ...
  /**
   * An SSO metadata URL is fetched server-side by the IdP handshake — require a
   * genuinely PUBLIC https endpoint. Reject http://, junk, and every loopback /
   * private / link-local / metadata host (not just single-label `localhost`) so
   * the "public hostname" the hint promises holds. The worker's SSRF guard is
