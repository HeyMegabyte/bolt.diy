  ...
  template: `
    <div class="p-7 flex-1 overflow-y-auto animate-fade-in max-md:p-4 space-y-6">
      ...
      <section class="card">
        ...
        @if (apiKeys().length === 0 && !creatingKey()) {
          <div class="p-6 text-center text-text-secondary text-sm">
            No API keys yet. Click <strong>Generate API key</strong> to create your first.
          </div>
        } @else {
...
export class AdminUserSettingsComponent implements OnInit {
  ...
  loadApiKeys(): void {
    ...
    this.api.get<{ data: ApiKeyRow[] }>('/admin/api-keys').subscribe({
      next: (r) => this.apiKeys.set(r.data ?? []),
  ...
  generateKey(): void {
    ...
    this.api.post<{ data: { name: string; secret: string; expires_at: string | null } }>('/admin/api-keys', {
      ...
      error: () => {
        this.generatingKey.set(false);
        this.toast.error('Could not generate key');
      },
    });
  }

  copyKey(secret: string): void {
    navigator.clipboard?.writeText(secret).then(() => this.toast.success('Copied'));
  }
  ...
  revokeKey(k: { id: string; name: string }): void {
    ...
    this.api.delete(`/admin/api-keys/${k.id}`).subscribe({
      next: () => { this.toast.success('Revoked'); this.loadApiKeys(); },
    });
  }
}
