      ...
      @if (!site()) {
        ...
        @if (loading()) {
          ...
          <app-empty-state icon="⚡" title="No automations yet"
            message="Create a recipe above to auto-run an action whenever a site event fires." />
        } @else {
          <ul class="flex flex-col gap-2">
            @for (r of recipes(); track r.id) {
              <li data-testid="recipes-row" class="flex items-center justify-between gap-3 rounded-lg bg-white/[0.03] px-3 py-2.5">
                ...
                <button hlmBtn variant="ghost" size="sm" data-testid="recipes-delete"
                        [attr.aria-label]="'Delete automation “' + r.name + '”'"
                        (click)="remove(r.id, r.name)">Delete</button>
              </li>
            }
          </ul>
        }
...
export class AdminRecipesComponent {
  ...
  /**
   * A webhook target is called server-side, so require https + a genuinely PUBLIC
   * host — reject http://, junk, and every loopback / private / link-local /
   * metadata host (not just single-label `localhost`) so the "public hostname"
   * the hint promises is enforced. The worker's SSRF guard is the real boundary.
   */
