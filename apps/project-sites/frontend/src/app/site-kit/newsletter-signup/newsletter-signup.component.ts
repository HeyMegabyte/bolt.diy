...
@Component({
  ...
  styles: [`
    .ns-input:focus { outline: 2px solid var(--ps-accent,#00e5ff); outline-offset: 2px; }
    .ns-btn:hover:not(:disabled) { opacity: .88; }
    .ns-btn:disabled { cursor: not-allowed; opacity: .5; }
    @media (prefers-reduced-motion: reduce) {
      .ns-btn { transition: none !important; }
    }
  `],
  template: `
    <section
      ...
      <div style="max-width:600px;margin:0 auto;text-align:center;">
        ...
        <h2 style="font-size:clamp(1.75rem,3.5vw,2.5rem);font-weight:700;
        ...
        <!-- Form -->
        <form *ngIf="!submitted()" (ngSubmit)="onSubmit()" #nsForm="ngForm" novalidate>
          ...
          <p *ngIf="disclaimer"
             style="margin:.875rem 0 0;font-size:.8rem;opacity:.45;">
            {{ disclaimer }}
          </p>
        </form>
      </div>
