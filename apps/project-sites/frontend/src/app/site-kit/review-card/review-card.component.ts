    ...
    >
      ...
      <footer style="display:flex;align-items:center;gap:12px;margin-top:4px;">
        <div
          ...
          "
          aria-hidden="true"
        >{{ reviewer.charAt(0) }}</div>
        <div>
          <div itemprop="name" style="color:var(--ps-ink,#f4f4ff);font-size:0.875rem;font-weight:700;">{{ reviewer }}</div>
          <div *ngIf="role" style="color:rgba(244,244,255,0.55);font-size:0.8rem;">{{ role }}</div>
        </div>
        <div *ngIf="platform" style="margin-left:auto;">
          <span style="
