...
@Component({
  ...
  template: `
    <section
      ...
      style="padding: 48px 24px; max-width: 700px; margin: 0 auto;"
    >
      @if (heading) {
        <h2
          [id]="headingId"
      ...
      @if (subtitle) {
        <p style="text-align:center;color:rgba(244,244,255,0.6);font-size:0.9rem;margin:0 0 32px;">
          {{ subtitle }}
        </p>
      }
...
export class ServiceAreaMapComponent {
  ...
  // (="zones.length"). The consumer passes the business's REAL zones. (anti-fabrication mandate)
