...
// EXCLUDED (real violations, but NOT fixable in this round):
//   /          + /contact  → `.text-primary/40` step numbers in
//                            homepage.component.html (CONCURRENT-SESSION-DIRTY
//                            file — owned by another session, hands off).
//   /signin    → `text-gray-500` buttons in signin.component.html
//                            (CONCURRENT-SESSION-DIRTY — hands off).
//   /status    → `link-in-text-block` on a `/health/deep` link that is NOT in
//                            the current source (runtime-injected/worker-served;
//                            elusive) — needs a focused investigation round.
//   /changelog → worker-served plaintext (flag-shadow; missing <title>/lang) —
//                            the Docker-blocked worker fix (round-87).
// Re-add each to ROUTES once its owning file is free / its fix lands.
