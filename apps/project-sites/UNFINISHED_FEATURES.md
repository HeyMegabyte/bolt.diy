...
### 11j. /loop iteration 4 (2026-06-08) — ai_auto_router route layer tested
Re-audited all 32 registry flags for genuine test gaps (filtering relative-import false-zeros: `gbp_assist`, `email_deliverability_wizard`, `search_engine_submit`→search_submit, `site_mcp_server`→mcp_site are all impl-tested; the flag-key string just doesn't appear in their specs). Genuine gap found: **`ai_auto_router`** — its service fns (`autoRoutePrompt`/`getRouterStats`) are thoroughly covered in `brilliant.test.ts`, but the **route layer** in `features.ts` (`POST /api/router/pick`, `GET /api/router/stats`) — the `requireFlag('ai_auto_router')` gate + request wiring — had ZERO tests.
- Added `src/__tests__/router_routes.test.ts` (6 tests) via the `features` sub-app `.request()`: flag-off → 404 `not_found` (no leak, service never called); flag-on → 200 routing the body `prompt`/`org_id` through to `autoRoutePrompt`; empty-body → `'demo prompt'` default; `/stats` flag-off 404, default `demo-org`, and `?org_id=` passthrough. Mocks only two boundaries (`isFlagOn` + the two brilliant fns), keeping the real Hono app.
- Establishes the route-layer test pattern for the remaining flag-gated `features.ts` grab-bag handlers (convert per-flag as each promotes, per CLAUDE.md — no mass retrofit).
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **233 suites / 4326 tests pass**.

### 11k. /loop iteration 5 (2026-06-08) — token_burn_meter fully tested (service + route)
Deeper per-module audit (mount + colocated-test-depth) surfaced that **`token_burn_meter`** was UNTESTED at BOTH layers: its only "colocated" test (`libs/features/token_burn_meter/__tests__/build_budget.test.ts`) actually covers an unrelated `build_budget` concern, and its real service fns (`src/services/features.ts`: `estimatePromptCost`, `pickModel`, `listModels`, `recordTokenEvent`, `getMonthlyBurn`) + its two flag-gated routes (`GET /api/usage/burn`, `POST /api/usage/record`) had zero coverage.
- Added `src/__tests__/token_burn_meter.test.ts` (14 tests): per-million pricing math (paid/free/unknown→sonnet fallback), shape→model selection (+ valid/invalid userPref), model catalog, `recordTokenEvent` INSERT with rounded cents (captured D1 stub), `getMonthlyBurn` per-model aggregation + zeroed-empty, and the route layer (flag-off 404, org_id default, body defaults, live wiring through the REAL service). Mocks only `isFlagOn`; the real feature service runs against a capturing D1 stub.
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **234 suites / 4340 tests pass**.

### 11l. /loop iteration 6 (2026-06-08) — pwa_manifest_full fully tested (stable LIVE flag)
Audited every exported fn in the `src/services/features.ts` grab-bag for test coverage. Picked the highest-priority gap: **`pwa_manifest_full`** is STABLE / default-on → its route `GET /api/pwa/manifest` is LIVE in prod, yet `getPwaManifest` + the route had ZERO coverage (iter 3 only covered the *served-site* manifest via build_validators, not this admin-PWA endpoint).
- Added `src/__tests__/pwa_manifest.test.ts` (7 tests): service locks the full-PWA completeness contract from `[[always]]` (maskable 512 icon, ≥3 screenshots spanning wide+narrow form factors, ≥3 shortcuts, share_target POST/multipart, file_handlers, protocol_handlers `web+*`) + route layer (flag-off 404 no-leak, flag-on returns the manifest). Mocks only `isFlagOn`; real service runs.
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **235 suites / 4347 tests pass**.

> Remaining untested `features.ts` service fns (per the iter-6 audit) are experimental-flag-gated grab-bag handlers (provisionDatabase, marketplace, agency, trust-center, perf/a11y wizards, observability, media-gen, etc.) — convert per-flag as each promotes, one cohesive feature per loop fire. No mass retrofit (CLAUDE.md).

### 11m. /loop iteration 7 (2026-06-08) — flag-resolution ENGINE tested + 2 drift findings diagnosed
Audit of `src/services/features.ts` (53 exports) + knip showed only **3 are wired** (`getMonthlyBurn`, `getPwaManifest`, `recordTokenEvent`); the other **44 are dead code** orphaned by the iter-11d/e flag trim (their flags + routes were removed). So the remaining "untested feature flags" are NOT live gaps — the LIVE surface is covered (iters 1–6). Two genuine items surfaced, both **deferred to a supervised quiet-tree session** (their fix-sites are concurrently `M` by other worktrees — editing now would clobber in-flight work, per the worktree-concurrency caution):
  1. **Dead-code drift:** 44 unused exports in `src/services/features.ts` (knip-confirmed, `F`-tagged). Remove in a quiet-tree session — its sole importer `src/routes/features.ts` is currently being edited by another session. tsc + full jest + knip make the deletion safe to verify when the tree is quiet.
  2. **Stale-cache bug:** the registry override-write path (`src/routes/features.ts:387`, `POST .../feature-flags/:key/override`) writes `flag_overrides` but never calls `invalidateFlagCache`, so `resolveFlag`'s 60s KV cache serves stale state — violating the documented "KV cache invalidates immediately" contract. One-line fix (`await invalidateFlagCache(c.env, key)` after the INSERT) deferred because `routes/features.ts` is concurrently modified. (The copilot override path reads `flag_overrides` directly — no KV cache — so it's unaffected.)
- Delivered THIS fire (safe, isolated, no contested files): `src/__tests__/feature_flags_resolution.test.ts` (12 tests) — direct unit coverage of the flag-resolution engine that was only ever exercised indirectly: `resolveFlag` (unknown→fail-closed with zero I/O, KV-hit short-circuits D1, cache-miss→registry fallback + writes cache, override precedence tenant>org>global), `isFlagOn` rollout gating (disabled/100/0/deterministic-partial), and `invalidateFlagCache` (lists `flag:<key>:` + deletes every match — locking the exact cache-bust contract the deferred fix #2 must call).
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **236 suites / 4359 tests pass**.

### 11n. /loop iteration 8 (2026-06-08) — flag↔route coherence guard + 3rd drift finding
Scanned every flag-gated route file (outside the concurrently-`M` features.ts). NEW genuine finding: three flags gate LIVE, BUILT, tested route features but are **absent from FLAG_REGISTRY**, so `resolveFlag` fail-closes → those features **404 permanently** in prod:
  - `approval_workflow` → `src/routes/review_links.ts` + `review_public.ts` (the #4 review/approval feature)
  - `multimodal_copilot` → `src/routes/copilot.ts`
  - `domain_stack_wizard` → `src/routes/domain_stack.ts`
  `validate-feature-manifests` didn't catch these because it only guards `manifest.flagKey ∈ registry`; these are inline route guards with no libs/features manifest. **Fix (register the 3 flags) is DEFERRED** — `registry.ts` is concurrently `M` (the active review/copilot/domain convergence session is editing it, almost certainly adding exactly these). Editing now clobbers their in-flight work.
- Delivered THIS fire (safe, isolated): `src/__tests__/flag_route_coherence.test.ts` (3 tests) — locks the 18 REGISTERED live-route flags against accidental trim-removal (the iter-11d/e failure mode that produced this gap), validates each carries the required definition fields, and asserts the 3 pending features are genuinely BUILT (route files exist) so the gap is provably "registry entry missing," not "feature unbuilt." Deliberately does NOT assert the pending trio's absence (would flake against the concurrent registration).
...
### 11p. /loop iteration 10 (2026-06-08) — shared flag-guard gating semantics tested
Flag-config surface still saturated by concurrent sessions (registry.ts + routes/features.ts both `M`, HEAD advanced again; 3 flags still unregistered) — deferred fixes (§11m/§11n) remain blocked. Targeted the highest-leverage CLEAN file instead: `src/lib/feature_guard.ts` is the shared gate for contacts_core / email_marketing / site_analytics / data_export + every public flag-gated route. Its sibling spec covered only the error envelopes + the unauthenticated short-circuit (returns before any flag lookup) — the actual flag-GATING branches were untested.
- Added `src/__tests__/feature_guard_gating.test.ts` (5 tests, `isFlagOn` mocked): `requireOrgFlag` flag-off→404 NOT_FOUND no-leak (+ asserts the scope passed to isFlagOn) and flag-on→returns `{orgId,userId}`; `requireFlag` (public gate) flag-off→404, flag-on→`true`, and scope-passthrough. Silences the expected `logFlagOff` warn for clean output.
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **239 suites / 4371 tests pass**.

### 11q. /loop iteration 11 (2026-06-08) — trust_center ROUTE layer fully tested
Flag-config surface still concurrently churning (registry.ts + routes/features.ts `M`; the §11n trio still unregistered — re-confirmed via targeted grep; a broad route-guard audit also ran but its regex under-matches, so trust the per-flag check). Verified `pseo_matrix_v2` IS registered (not a 4th gap). Targeted a CLEAN gap: `routes/trust_center.ts` (6 handlers, flag `trust_center`) had its schemas + service fns tested (11 cases) but **zero route-layer coverage**.
- Added `src/__tests__/trust_center_routes.test.ts` (11 tests) via the shared harness (real isFlagOn through flagKv; service fns + the public-route site lookup mocked at their boundaries): GET /profile (unauth 401 / flag-off 404 / 200), POST /profile/publish (no-profile 404 / 200), PUT /site/:id (foreign-or-missing-site 404 tenant-isolation / 200 upsert), GET /public/trust/:slug (site-not-found 404 / flag-off 404 / not-published 404 / 200 with redacted profile + DigitalDocument JSON-LD).
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **240 suites / 4382 tests pass**.

### 11r. /loop iteration 12 (2026-06-08) — section_marketplace submissions routes (had NO test file)
Flag-config surface still concurrently churning (deferred §11m/§11n unchanged). Handler-vs-case scan of all own-file route modules found one with ZERO coverage: `routes/section_marketplace_submissions.ts` (creator-submit + admin-curation, flag `section_marketplace` — registered) had no test file at all (every other module — automation/webhooks/inbox/forms/pseo/section_marketplace — is well covered).
- Added `src/__tests__/section_marketplace_submissions_routes.test.ts` (12 tests) via the shared harness (real isFlagOn + real SectionSubmissionSchema; only the 3 service fns mocked): POST /sections (401 / flag-off 404 / bad-JSON 400 / schema-validation 400 / 201 success / 500 on service throw), GET /sections/pending (200 + count, limit NaN→100 default), POST /sections/:id/review (bad-decision 400 / 200 approve / SUBMISSION_NOT_FOUND→404 / SUBMISSION_NOT_PENDING→409).
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **241 suites / 4394 tests pass**.

### 11s. /loop iteration 13 (2026-06-08) — media route layer fully tested (10 handlers)
Confirmed every FLAG-GATED route module now has route-layer coverage (iters 1–12). A full route-file scan found the media library — a real "other feature" (not flag-gated) — was the last big untested route surface: `media.test.ts` is SERVICE-level only (hits zero `/api/media/*` endpoints), leaving all 10 route handlers + the `errorResponse` status-mapping table untested. (`email_deliverability`/`site_branches`/`wallet` flagged by the scan are false-negatives — all have tests.)
- Added `src/__tests__/media_routes.test.ts` (25 tests) via the shared harness (org auth injected; media service + R2 bucket mocked): GET /assets (401 / 200 + filter passthrough), GET /assets/:id (404/200), GET /assets/:id/raw (asset-404 / r2-object-404 / 200 stream w/ content-type), POST /upload (401 / non-multipart 400), DELETE (not-found 404 / 200), stock/search (blank-query 400 / 200), stock/save (missing-candidate 400 / 201 / TOO_LARGE→413), generate/image (blank-prompt 400 / 201 / OPENAI→503), generate/video (202), generate/podcast (missing-title 400 / empty-script 400 / 201), send-to-bolt (missing-assetId 400 / ASSET_NOT_FOUND→404). Locks the full service-error→HTTP-status table.
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **242 suites / 4419 tests pass**.

### 11t. /loop iteration 14 (2026-06-08) — wallet route layer tested + §11m/§11n now owned by an in-flight refactor
Inspected the uncommitted `registry.ts`/`features.ts` diff: a concurrent session is mid-way through the **big flag-trim** (131-line registry deletion + 431-line features.ts deletion — removing ~40 orphaned experimental flags incl. `approval_workflow`, adding `core_billing`). So the §11m dead-code + §11n missing-flags items are being resolved by THAT refactor — confirmed hands-off (touching = guaranteed conflict). Notably it's *removing* `approval_workflow` (not registering it), which vindicates the coherence guard never asserting the trio present.
- Picked a slice fully OUTSIDE the refactor scope: `routes/wallet.ts` (billing-core, not flag-gated). Its 4 test files (75 cases) are all service-level — the 5 route handlers were untested. Added `src/__tests__/wallet_routes.test.ts` (15 tests) via a Hono harness wired with the real `errorHandler` (so `throw unauthorized()` → 401 faithfully): GET /wallet (401/200), POST /subscribe (401 / invalid-url 400 / checkout-fail 400 / 200), POST /topup (401 + amount clamp default-5000/min-100/max-50000/passthrough), GET /transactions (401/200/days-clamp-365), GET /cost-categories (public 200). Service + D1 mocked.
- Test-only — no worker source changed, no deploy needed. Verified: `tsc` 0 · `validate:features` 0 errors (4 pre-existing warnings) · full suite **243 suites / 4436 tests pass**.

> Status: flag-gated + the two big non-flag "other feature" route surfaces (media, wallet) are now all route-layer covered. The remaining flag-registry work is actively owned by the concurrent trim refactor — the loop should not contend for it.
